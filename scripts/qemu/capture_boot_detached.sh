...
OUT_PPM="$OUTPUT_DIR/exoos-qemu-detached.ppm"
...
src = Path('/tmp/exoos-screen.ppm')
dst_ppm = Path(os.environ['OUT_PPM'])
dst = Path(os.environ['OUT_PNG'])
data = src.read_bytes()
dst_ppm.write_bytes(data)
idx = 2
tokens = []
while len(tokens) < 3:
...
printf "\nCAPTURE_PNG=%s" "$OUT_PNG"
printf "\nCAPTURE_PPM=%s" "$OUT_PPM"
