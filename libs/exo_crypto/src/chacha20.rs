...
mod tests {
    ...
    fn test_chacha20_roundtrip() {
        let key = [42u8; 32];
        let nonce = [17u8; 12];
        let plaintext = b"Hello, Exo-OS! This is a test message for ChaCha20 encryption.";
        ...
        // Déchiffrement
        let mut decipher = ChaCha20::new(&key, &nonce, 0);
    ...
    fn test_xchacha20_roundtrip() {
        let key = [42u8; 32];
        let nonce = [17u8; 24];
        let plaintext = b"Hello, Exo-OS! This is a test message for XChaCha20 encryption.";
        ...
        // Déchiffrement
        let mut decipher = XChaCha20::new(&key, &nonce);
        decipher.process(&mut data);
    ...
    #[test]
    fn test_xchacha20_aead() {
        let key = [42u8; 32];
        let nonce = [17u8; 24];
        let plaintext = b"Hello, Exo-OS! This is a test message for XChaCha20-Poly1305.";
        let aad = b"Associated authenticated data";

        let mut ciphertext = vec![0u8; plaintext.len() + POLY1305_TAGBYTES];
        assert!(XChaCha20::encrypt_aead(
            plaintext,
            aad,
            &nonce,
            &key,
        ...
        // Test avec AAD incorrect
        let mut decrypted_bad = vec![0u8; plaintext.len()];
        assert!(!XChaCha20::decrypt_aead(
            &ciphertext,
            b"Wrong AAD",
            &nonce,
