...
mod tests {
    ...
    impl Serialize for TestMessage {
        fn serialize(&self, buffer: &mut [u8]) -> Result<usize, &'static str> {
            ...
            buffer[..size_of::<Self>()].copy_from_slice(bytes);
            Ok(size_of::<Self>())
        }
        ...
        fn deserialize(buffer: &[u8]) -> Result<Self, &'static str> {
            ...
            let bytes = unsafe {
                core::slice::from_raw_parts_mut(&mut msg as *mut Self as *mut u8, size_of::<Self>())
            };

            bytes.copy_from_slice(&buffer[..size_of::<Self>()]);
            Ok(msg)
        }
    }
    ...
    fn test_message_flags() {
        let mut flags = MessageFlags::new();
        assert!(!flags.contains(MessageFlags::INLINE));
        
        flags.set(MessageFlags::INLINE);
        assert!(flags.contains(MessageFlags::INLINE));
        
        flags.set(MessageFlags::ZERO_COPY);
        assert!(flags.contains(MessageFlags::ZERO_COPY));
        
        flags.unset(MessageFlags::INLINE);
        assert!(!flags.contains(MessageFlags::INLINE));
        assert!(flags.contains(MessageFlags::ZERO_COPY));
    }
    ...
    fn test_message_serialization() {
        let test_msg = TestMessage {
            value: 42,
            text: *b"Hello Exo-OS!\0\0\0",
        };
        
        let msg = Message::from_serializable(&test_msg).unwrap();
        assert!(msg.is_inline());
        
        let data = msg.inline_data();
        let deserialized: TestMessage = TestMessage::deserialize(data).unwrap();
