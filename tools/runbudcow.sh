#!/bin/bash
# Probe BUDCOW : le buddy rend-il une frame encore CoW-mappée (= collision) ?
...
LOG=tools/e9budcow.txt; rm -f "$LOG"
...
echo "=== BUDCOW (collision : buddy rend une frame CoW-mappée) ==="
tr -cd '\11\12\15\40-\176' < "$LOG" | grep -aoE '<BUDCOW p=[0-9a-f]+ rc=[0-9]+ o=[0-9]>'
echo "=== BUDCOW count + frames uniques ==="
tr -cd '\11\12\15\40-\176' < "$LOG" | grep -aoE '<BUDCOW p=[0-9a-f]+' | sort | uniq -c
