...
touch kernel/src/fs/exofs/storage/virtio_adapter.rs
...
echo "=== BPG (alloc/free page DMA 0xff33000) : doit S ARRETER apres warmup (#26 OK) ==="
echo "total BPG: $(tr -cd '\11\12\15\40-\176' < "$LOG" | grep -aoc '<BPG')"
echo "=== EXEC serveurs (markers kernel E9) ==="
tr -cd '\11\12\15\40-\176' < "$LOG" | grep -aoE '<EXEC [^>]*>' | sort | uniq -c
echo "=== logs serveurs (X: Y via SYS_EXO_LOG) ==="
tr -cd '\11\12\15\40-\176' < "$LOG" | grep -aoE '[a-z_]+: [a-zA-Z_ ]+' | grep -avE 'boot_display' | sort | uniq -c | head -25
echo "=== panic/fault/timeout ? ==="
tr -cd '\11\12\15\40-\176' < "$LOG" | grep -aoiE 'panic|timeout|fault|degraded|rescue' | sort | uniq -c | head
