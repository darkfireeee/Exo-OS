#!/bin/bash
# Vérif finale : compile propre après retrait de toutes les sondes #25 + baseline boot.
...
make iso >/tmp/mk.log 2>&1 && echo "ISOOK" || { echo "BUILDFAIL"; grep -iE "error\[|error:|warning: unused" /tmp/mk.log | head -25; exit 1; }
cargo run -q -p exofs-mkroot -- --image target/qemu/exofs-root.img --size 536870912 --root target/exofs-rootfs 2>&1 | tail -1
LOG=tools/e9verif.txt; rm -f "$LOG"
...
echo "=== baseline boot ==="
tr -cd '\11\12\15\40-\176' < "$LOG" | grep -aoE 'boot_display: (boot complete|userspace)|init_server: [a-zA-Z0-9 ]+|init: [a-zA-Z_/, =0-9-]+|ipc_router: [a-z]+|exosh' | tail -12
echo "=== anomalies (SEGV/panic/FA résiduel) ==="
tr -cd '\11\12\15\40-\176' < "$LOG" | grep -aocE '<SEGV>|panic|<FA |<F1 |DIAG'
