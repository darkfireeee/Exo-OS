...
LOG=tools/e9long2.txt; rm -f "$LOG"
...
echo "=== progression (init + serveurs + shell) ==="
tr -cd '\11\12\15\40-\176' < "$LOG" | grep -aoE 'init[_a-z]*: [a-zA-Z_/, =0-9-]+|[a-z_]+_server: [a-zA-Z ]+|exosh[^<]+|chk [a-z_]+|<EXEC [^>]*>' | sort | uniq -c | head -40
echo "=== K0/LKP (readiness) ==="
tr -cd '\11\12\15\40-\176' < "$LOG" | grep -aoc '<K0 \|<LKP '
