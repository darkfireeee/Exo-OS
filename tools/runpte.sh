#!/bin/bash
# Probe PTE (anomalies) : faute read/exec d'init sur page présente — flags AS vs CR3 hw.
...
tr -cd '\11\12\15\40-\176' < "$LOG" | grep -aoE 'init: [a-zA-Z_/, =0-9-]+|ipc_router: [a-z]+' | tail -4
echo "=== PTE ANOMALIES (x@/r@ addr ; as/hw flags P W U X=NX) ==="
tr -cd '\11\12\15\40-\176' < "$LOG" | grep -aoE '<PTE [xr]@[0-9a-f]+ as=[PWUX-]+ hw=[PWUX-]+>'
echo "=== uniq ==="
tr -cd '\11\12\15\40-\176' < "$LOG" | grep -aoE '<PTE [xr]@[0-9a-f]+ as=[PWUX-]+ hw=[PWUX-]+>' | sort | uniq -c
