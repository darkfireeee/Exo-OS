#!/bin/bash
# Probe RDY2 : pour ipc_router, quel check de readiness échoue (alive vs endpoint).
...
touch servers/init_server/src/*.rs kernel/src/scheduler/core/switch.rs kernel/src/arch/x86_64/exceptions.rs
...
echo "=== logs init/ipc ==="
tr -cd '\11\12\15\40-\176' < "$LOG" | grep -aoE 'init: [a-zA-Z_/, +=0-9-]+|ipc_router: [a-z]+' | tail -20
echo "=== RDY2 (A=alive E=endpoint) uniq ==="
tr -cd '\11\12\15\40-\176' < "$LOG" | grep -aoE 'init: rdy [a-z_]+ [A-Za-z,+]+' | sort | uniq -c
