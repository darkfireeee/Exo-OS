...
pub const SERVER_ENDPOINT_ID: u64 = 8;
pub const IPC_RECV_TIMEOUT_MS: u64 = 5_000;

pub const SCHED_MSG_HEARTBEAT: u32 = 0;
pub const SCHED_MSG_THREAD_REGISTER: u32 = 1;
pub const SCHED_MSG_SET_PRIORITY: u32 = 2;
pub const SCHED_MSG_SET_POLICY: u32 = 3;
pub const SCHED_MSG_SET_AFFINITY: u32 = 4;
pub const SCHED_MSG_YIELD: u32 = 5;
pub const SCHED_MSG_GET_STAT: u32 = 6;
pub const SCHED_MSG_REALTIME_ADMIT: u32 = 7;
pub const SCHED_MSG_REALTIME_RELEASE: u32 = 8;
...
const _: () = assert!(core::mem::size_of::<SchedulerRequest>() == syscall::IPC_ENVELOPE_SIZE);
const _: () = assert!(core::mem::offset_of!(SchedulerRequest, payload) == syscall::IPC_HEADER_SIZE);
...
pub fn recv_request(request: &mut SchedulerRequest) -> Result<bool, i64> {
    // SAFETY: le noyau écrit dans `request`, taille bornée à la struct ABI.
    let rc = unsafe {
        syscall::syscall4(
            syscall::SYS_IPC_RECV,
            SERVER_ENDPOINT_ID,
            request as *mut SchedulerRequest as u64,
            core::mem::size_of::<SchedulerRequest>() as u64,
...
pub fn send_reply(destination_pid: u32, reply: &SchedulerReply) -> i64 {
    // SAFETY: `reply` est une structure POD locale envoyée telle quelle au noyau.
    unsafe {
        syscall::syscall6(
            syscall::SYS_IPC_SEND,
            destination_pid as u64,
            reply as *const SchedulerReply as u64,
            core::mem::size_of::<SchedulerReply>() as u64,
            0,
            0,
            0,
        )
