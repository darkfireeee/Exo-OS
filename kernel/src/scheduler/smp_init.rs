...
pub fn init_smp_scheduler() {
    ...
    info!("[SMP Scheduler] Initializing for {} CPUs", cpu_count);
    ...
    if cpu_count > 1 {
        ...
        info!("[SMP Scheduler] Creating idle threads...");
        for cpu_id in 0..cpu_count {
            ...
            if let Some(queue) = PER_CPU_QUEUES.get(cpu_id) {
                ...
                info!("[SMP Scheduler] CPU {} idle thread ready", cpu_id);
            }
        }
        
        info!("[SMP Scheduler] Load balancing enabled");
    } else {
        info!("[SMP Scheduler] Single CPU mode");
        ...
        if let Some(queue) = PER_CPU_QUEUES.get(0) {
            queue.set_current_thread(Some(idle_thread));
            info!("[SMP Scheduler] BSP idle thread ready");
        }
    }
}
