...
pub fn lapic_write(reg: u32, val: u32) {
    ...
    unsafe { write_volatile((base + reg as usize) as *mut u32, val); }
}
...
pub fn enable_xapic() {
    ...
    unsafe { msr::write_msr(MSR_IA32_APICBASE, new_base); }
...
pub fn set_spurious_vector(vector: u8) {
    ...
    lapic_write(LAPIC_SIVR, (svr & 0xFFFF_FF00) | SIVR_APIC_ENABLE | (vector as u32));
}
...
pub fn timer_set_deadline(deadline_tsc: u64) {
    ...
    unsafe { msr::write_msr(msr::MSR_TSC_DEADLINE, deadline_tsc); }
}
...
pub fn calibrate_lapic_timer() {
    if TIMER_CALIBRATED.load(Ordering::Acquire) { return; }
...
fn icr_wait_delivery() {
    loop {
        ...
        if icr_low & ICR_PENDING == 0 { break; }
        core::hint::spin_loop();
    }
}
...
pub fn send_ipi(dest_apic_id: u8, vector: u8, delivery_mode: u32) {
    ...
    lapic_write(LAPIC_ICR_LOW, ICR_LEVEL_ASSERT | delivery_mode | (vector as u32));
}
...
pub fn broadcast_ipi_except_self(vector: u8) {
    ...
    lapic_write(LAPIC_ICR_LOW, ICR_DEST_OTHER | ICR_LEVEL_ASSERT | ICR_DM_FIXED | (vector as u32));
}
...
pub fn send_init_ipi(dest_apic_id: u8) {
    icr_wait_delivery();
    lapic_write(LAPIC_ICR_HIGH, (dest_apic_id as u32) << 24);
    lapic_write(LAPIC_ICR_LOW, ICR_LEVEL_ASSERT | ICR_TRIGGER_LEVEL | ICR_DM_INIT);
}
...
pub fn send_startup_ipi(dest_apic_id: u8, page: u8) {
    icr_wait_delivery();
    lapic_write(LAPIC_ICR_HIGH, (dest_apic_id as u32) << 24);
    lapic_write(LAPIC_ICR_LOW, ICR_LEVEL_ASSERT | ICR_DM_STARTUP | (page as u32));
}
...
static TIMER_CALIBRATED:   core::sync::atomic::AtomicBool = core::sync::atomic::AtomicBool::new(false);

/// Fréquence du timer LAPIC en ticks/µs (après calibration)
pub fn lapic_timer_freq_us()  -> u32 { TIMER_TICKS_PER_US.load(Ordering::Relaxed) }
pub fn lapic_timer_freq_ms()  -> u32 { TIMER_TICKSPER_MS.load(Ordering::Relaxed) }
pub fn lapic_timer_calibrated() -> bool { TIMER_CALIBRATED.load(Ordering::Relaxed) }
