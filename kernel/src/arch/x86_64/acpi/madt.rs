...
pub fn parse_madt(madt_phys: u64) -> MadtInfo {
    ...
    if madt_len < madt_base_offset + 8 || madt_len > 65536 {
        return info;
    }
    ...
    while offset + 2 <= madt_len {
        ...
        match entry_type {
            MADT_TYPE_LAPIC => {
                if entry_len < 2 + core::mem::size_of::<MadtLapic>() {
                    offset += entry_len;
                    continue;
                }
            ...
            MADT_TYPE_IOAPIC => {
                if entry_len < 2 + core::mem::size_of::<MadtIoApic>() {
                    offset += entry_len;
                    continue;
                }
                let base = entry_base as *const u8;
            ...
            MADT_TYPE_INT_SRC_OVER => {
                if entry_len < 2 + core::mem::size_of::<MadtIntSrcOvr>() {
                    offset += entry_len;
                    continue;
                }
                let base = entry_base as *const u8;
            ...
            MADT_TYPE_LAPIC_ADDR_OVR => {
                if entry_len < 2 + core::mem::size_of::<MadtLapicAddrOvr>() {
                    offset += entry_len;
                    continue;
                }
                let base = entry_base as *const u8;
            ...
            MADT_TYPE_X2APIC => {
                if entry_len < 2 + core::mem::size_of::<MadtX2Apic>() {
                    offset += entry_len;
                    continue;
                }
                let base = entry_base as *const u8;
