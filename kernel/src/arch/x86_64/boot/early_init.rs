...
impl BootInfo {
    const fn zeroed() -> Self {
        Self {
            rsdp_phys: 0, multiboot2_magic: 0, multiboot2_addr: 0,
            uefi_memmap_addr: 0, uefi_memmap_size: 0, uefi_desc_size: 0,
            total_memory_kb: 0, cpu_count: 1, lapic_base: 0xFEE00000,
        }
    }
}
...
pub unsafe fn arch_boot_init(
    ...
    let rsdp = if rsdp_phys != 0 {
        ...
        match super::super::acpi::parser::find_rsdp() {
            Some(addr) => { super::super::acpi::parser::init_acpi_from_rsdp(addr); addr }
            None       => 0,
        }
    };
    ...
    if mb2_magic == 0x36d76289 && mb2_info != 0 {
        ...
        boot_info.rsdp_phys        = if mb2.rsdp_phys != 0 { mb2.rsdp_phys } else { rsdp };
        ...
        crate::memory::protection::init();    } else if mb2_magic == super::memory_map::EXOBOOT_MAGIC_U32 && mb2_info != 0 {
        ...
        let rsdp_from_bi = core::ptr::read_volatile(
            (mb2_info + 6200) as *const u64
        );
        if rsdp_from_bi != 0 {
