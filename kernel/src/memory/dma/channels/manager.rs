...
use crate::memory::dma::core::types::{DmaCapabilities, DmaChannelId, DmaError, DmaPriority};
...
impl ChannelQueue {
    ...
    fn push(&mut self, txn: DmaTransactionId) -> bool {
        if self.count >= CHANNEL_QUEUE_DEPTH {
            return false;
        }
    ...
    fn is_full(&self) -> bool {
        self.count >= CHANNEL_QUEUE_DEPTH
    }
    fn len(&self) -> usize {
        self.count
    }
...
impl DmaChannel {
    ...
    pub fn is_available(&self) -> bool {
        self.registered.load(Ordering::Acquire) && matches!(self.state(), ChannelState::Idle)
    }
...
impl DmaChannelManager {
    ...
    pub fn alloc_channel(
        ...
        for i in 0..count {
            ...
            if ch.is_available() && ch.supports(required_caps) && ch.priority >= min_priority {
    ...
    pub fn channel(&self, id: DmaChannelId) -> Option<&DmaChannel> {
        if (id.0 as usize) < MAX_DMA_CHANNELS {
            ...
            if ch.registered.load(Ordering::Acquire) {
                return Some(ch);
            }
        }
        None
