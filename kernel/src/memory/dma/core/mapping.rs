...
use crate::memory::dma::core::types::{
    IovaAddr, DmaDirection, DmaMapFlags, DmaError, IommuDomainId,
};
...
impl DmaMapping {
    fn new(
        phys_base: PhysAddr, iova_base: IovaAddr,
        size: usize, direction: DmaDirection,
        flags: DmaMapFlags, domain: IommuDomainId,
    ) -> Self {
        DmaMapping {
            phys_base, iova_base, size, direction, flags, domain,
    ...
    #[inline]
    pub fn ref_count(&self) -> usize { self.ref_count.load(Ordering::Relaxed) }
}
...
impl IovaAllocator {
    ...
    pub fn enable_iommu(&self) { self.passthrough.store(0, Ordering::Release); }
    ...
    pub fn map(
        ...
        if self.is_passthrough() {
            ...
            self.stats_bytes.fetch_add(size_aligned as u64, Ordering::Relaxed);
            return Ok(iova);
        }

        // Mode IOMMU : alloue une plage IOVA.
        let mut inner = self.inner.lock();
        if inner.count >= MAX_DMA_MAPPINGS { return Err(DmaError::OutOfMemory); }
        ...
        let slot = inner.active.iter_mut().find(|s| s.is_none())
            .ok_or(DmaError::OutOfMemory)?;
        *slot = Some(DmaMapping::new(phys, iova, size_aligned, direction, flags, domain));
        inner.count += 1;
    ...
    pub fn map_sg(
        ...
        if entries.len() > out_iovas.len() { return Err(DmaError::InvalidParams); }
