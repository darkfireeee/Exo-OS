...
impl Default for AhciSlotStatus {
    fn default() -> Self { AhciSlotStatus { occupied: false, write_op: false, size_bytes: 0 } }
}
...
impl AhciDmaEngine {
    ...
    pub unsafe fn init(&self, hba_base: u64, port: u32) -> bool {
        ...
        let pi = (hba_base as *const u32).add(hba_regs::PI / 4).read_volatile();
        if pi & (1 << port) == 0 { return false; }
    ...
    unsafe fn submit_dma(
        ...
        let ctba_phys = PhysAddr::new(
            &st.cmd_tables[slot] as *const _ as u64
        );
        ...
        // Marquer occupé.
        st.slots[slot] = AhciSlotStatus { occupied: true, write_op: write, size_bytes };
    ...
    pub unsafe fn poll(&self) -> usize {
        ...
        for slot in 0..AHCI_SLOTS {
            if !st.slots[slot].occupied { continue; }
            ...
            if ci & (1 << slot) == 0 {
                ...
                if tfd & 0x01 != 0 {
                    ...
                    dma_stat_complete(eid, st.slots[slot].size_bytes as u64, st.slots[slot].write_op, 0);
                }
