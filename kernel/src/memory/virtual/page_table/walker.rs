...
use crate::memory::virt::page_table::x86_64::{
    ...
    phys_to_table_mut, phys_to_table_ref,
};
...
impl PageTableWalker {
    ...
    pub fn walk_read(&self, virt: VirtAddr) -> WalkResult {
        ...
        if !l4_entry.is_present() { return WalkResult::NotMapped; }
        ...
        if !l3_entry.is_present() { return WalkResult::NotMapped; }
        if l3_entry.is_huge() {
            return WalkResult::HugePage { entry: l3_entry, level: PageTableLevel::L3 };
        }
        ...
        if !l2_entry.is_present() { return WalkResult::NotMapped; }
        if l2_entry.is_huge() {
            return WalkResult::HugePage { entry: l2_entry, level: PageTableLevel::L2 };
        }
        ...
        if !l1_entry.is_present() { return WalkResult::NotMapped; }
        WalkResult::Leaf { entry: l1_entry, level: PageTableLevel::L1 }
    }
    ...
    pub fn unmap(&mut self, virt: VirtAddr) -> Option<Frame> {
        ...
        let pml4     = unsafe { phys_to_table_mut(self.pml4_phys) };
        let l4_entry = pml4[virt.p4_index()];
        if !l4_entry.is_present() { return None; }
    ...
    pub fn remap_flags(&mut self, virt: VirtAddr, new_flags: PageFlags) -> Result<(), AllocError> {
        ...
        let pml4     = unsafe { phys_to_table_mut(self.pml4_phys) };
        let l4_entry = pml4[virt.p4_index()];
        if !l4_entry.is_present() { return Err(AllocError::InvalidParams); }
        ...
        if !entry.is_present() { return Err(AllocError::InvalidParams); }
    ...
    fn ensure_table<A: FrameAllocatorForWalk>(
        ...
        let parent_flags =
            ...
            if flags.contains(PageFlags::USER) { PageTableEntry::FLAG_USER } else { 0 };
