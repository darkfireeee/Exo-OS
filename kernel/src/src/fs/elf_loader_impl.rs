...
use crate::memory::{
    PhysAddr, VirtAddr, Frame, PAGE_SIZE, AllocFlags, phys_to_virt,
};
...
impl ElfLoader for ExoFsElfLoader {
    fn load_elf(
        ...
        let mut builder = PageTableBuilder::new(&alloc)
        ...
        unsafe { builder.copy_kernel_entries(); }
...
fn map_elf_segment<A: FrameAllocatorForWalk>(
    ...
    for page_idx in 0..n_pages {
        let frame = buddy::alloc_pages(0, AllocFlags::ZEROED)
            .map_err(|_| ElfLoadError::OutOfMemory)?;
            ...
            if file_end <= elf_data.len() {
                ...
                let dst = unsafe {
                    core::slice::from_raw_parts_mut(
                        frame_virt.as_u64() as *mut u8,
                        PAGE_SIZE,
                    )
                };
                dst[dst_off..dst_off + copy_len]
                    .copy_from_slice(&elf_data[file_start..file_end]);
            }
        }

        let virt = VirtAddr::new(
            vaddr_page.saturating_add((page_idx * PAGE_SIZE) as u64)
        );
        builder.map_page(virt, frame, page_flags)
            .map_err(|_| ElfLoadError::OutOfMemory)?;
    }
...
fn map_stack_pages<A: FrameAllocatorForWalk>(
    ...
    for page_idx in 0..n_pages {
        let frame = buddy::alloc_pages(0, AllocFlags::ZEROED)
            .map_err(|_| ElfLoadError::OutOfMemory)?;
        let virt = VirtAddr::new(
            stack_base.saturating_add((page_idx * PAGE_SIZE) as u64)
        );
        builder.map_page(virt, frame, PageFlags::USER_DATA)
            .map_err(|_| ElfLoadError::OutOfMemory)?;
    }
    Ok(())
}
