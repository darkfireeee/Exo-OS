...
fn list_dir_entries(fd: u32, max_entries: usize) -> ExofsResult<Vec<ExofsDirEntry>> {
    ...
    result.try_reserve(count).map_err(|_| ExofsError::NoMemory)?;
    // RECUR-01 : while
    let mut i = 0usize;
    while i < count {
        let e = &all_entries[i];
        ...
        name_vec.try_reserve(name_bytes.len()).map_err(|_| ExofsError::NoMemory)?;
        let mut j = 0usize;
...
fn serialize_dirents(entries: &[ExofsDirEntry], buf_len: usize) -> ExofsResult<Vec<u8>> {
    let mut out = Vec::new();
    out.try_reserve(buf_len.min(EXOFS_LIST_MAX)).map_err(|_| ExofsError::NoMemory)?;
    let mut i = 0usize;
    while i < entries.len() {
        let entry = &entries[i];
        ...
        if out.len().saturating_add(reclen) > buf_len { break; }
        // Écrire le header
        ...
        let padding = reclen.saturating_sub(HEADER_SIZE).saturating_sub(name_len).saturating_sub(1);
        let mut p = 0usize;
        while p < padding {
            out.push(0u8);
...
pub fn sys_exofs_readdir(
    ...
    if let Err(e) = validate_user_ptr(buf_ptr, buf_len as usize) { return e; }
    // Valider le fd
