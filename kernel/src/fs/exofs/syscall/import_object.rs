...
use super::export_object::{
    EXPORT_HDR_SIZE, check_export_header, extract_payload,
};
...
fn read_user_buf(ptr: u64, len: u64) -> ExofsResult<Vec<u8>> {
    if ptr == 0 { return Err(ExofsError::InvalidArgument); }
    if len > IMPORT_MAX_SIZE { return Err(ExofsError::InvalidArgument); }
...
pub fn sys_exofs_import_object(
    ...
    let (blob_id, bytes_stored) = if args.flags & import_flags::RAW != 0 {
        match import_raw(&data, args.flags) { Ok(r) => r, Err(e) => return exofs_err_to_errno(e) }
    } else {
        match import_from_export(&data, args.flags) { Ok(r) => r, Err(e) => return exofs_err_to_errno(e) }
    };

    let path = if args.path_ptr != 0 && args.path_len > 0 {
        match read_user_buf(args.path_ptr, args.path_len) { Ok(v) => v, Err(e) => return exofs_err_to_errno(e) }
    } else { Vec::new() };
    ...
    if result_ptr != 0 {
        ...
        let mut i = 0usize;
        while i < 32 { bid_arr[i] = bid_bytes[i]; i = i.wrapping_add(1); }
        let res = ImportResult { blob_id: bid_arr, bytes_stored, fd, flags: args.flags };
        // SAFETY: pointeur valide sur une struct repr(C), durée de vie bornée par la référence.
        let bytes = unsafe { core::slice::from_raw_parts(&res as *const ImportResult as *const u8, core::mem::size_of::<ImportResult>()) };
...
pub fn already_imported(data: &[u8], raw: bool) -> bool {
    let bid = if raw {
        ...
        match extract_payload(data) { Ok(p) => BlobId::from_bytes_blake3(p), Err(_) => return false }
    };
...
pub fn batch_import(payloads: &[&[u8]], flags: u32) -> ExofsResult<Vec<BlobId>> {
    ...
    ids.try_reserve(payloads.len()).map_err(|_| ExofsError::NoMemory)?;
    let mut i = 0usize;
...
mod tests {
    ...
    #[test]
    fn test_import_args_size() { assert_eq!(core::mem::size_of::<ImportArgs>(), 40); }

    #[test]
    fn test_import_result_size() { assert_eq!(core::mem::size_of::<ImportResult>(), 48); }
    ...
    fn test_import_invalid_flags() {
        ...
        let args = ImportArgs { flags: 0, _pad: 0, path_ptr: 0, path_len: 0, data_ptr: 0, data_len: 0 };
    ...
    #[test]
    fn test_import_max_size_const() { assert_eq!(IMPORT_MAX_SIZE, 256 * 1024 * 1024); }

    #[test]
    fn test_import_flags_valid_mask() {
        assert_eq!(import_flags::VALID_MASK, import_flags::OVERWRITE | import_flags::VERIFY | import_flags::RAW | import_flags::OPEN_FD);
    }
...
pub fn import_copy_with_salt(src: &BlobId, salt: &[u8]) -> ExofsResult<BlobId> {
    ...
    combined.try_reserve(data.len().saturating_add(salt.len())).map_err(|_| ExofsError::NoMemory)?;
    let mut i = 0usize;
    while i < data.len() { combined.push(data[i]); i = i.wrapping_add(1); }
    let mut i = 0usize;
...
pub fn imports_are_equal(a: &[u8], b: &[u8], raw: bool) -> ExofsResult<bool> {
    ...
    let mut diff = 0u8;
    let mut i = 0usize;
    while i < 32 { diff |= ba[i] ^ bb[i]; i = i.wrapping_add(1); }
    Ok(diff == 0)
}
