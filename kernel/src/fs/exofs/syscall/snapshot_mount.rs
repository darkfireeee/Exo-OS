...
fn mount_snapshot(
    snap_blob_id: BlobId,
    args: &SnapshotMountArgs,
) -> ExofsResult<SnapshotMountResult> {
    if args.flags & !mount_flags::VALID_MASK != 0 {
        return Err(ExofsError::InvalidArgument);
    }
    let snap_data = BLOB_CACHE
        .get(&snap_blob_id)
    ...
    if args.epoch_id != 0 && epoch != args.epoch_id {
...
pub fn make_mount_entry(r: &SnapshotMountResult, flags: u32) -> MountEntry {
    ...
    let mut i = 0usize;
    while i < 32 {
        e.snapshot_id[i] = r.snapshot_id[i];
        i = i.wrapping_add(1);
    }
    e
}
...
pub fn snapshot_content_size(snap_id: BlobId) -> usize {
    let data = match BLOB_CACHE.get(&snap_id) {
        Some(d) => d,
        None => return 0,
    };
...
mod advanced_tests {
    ...
    fn test_make_mount_entry() {
        let r = SnapshotMountResult {
            fd: 5,
            epoch_id: 3,
            ..SnapshotMountResult::default()
        };
