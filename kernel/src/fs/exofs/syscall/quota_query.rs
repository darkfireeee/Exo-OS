...
pub fn get_quota(owner_uid: u64) -> ExofsResult<QuotaInfo> {
    ...
    Ok(QuotaInfo {
        used_bytes: 0,
        soft_bytes: QUOTA_UNLIMITED,
        hard_bytes: QUOTA_UNLIMITED,
        used_objects: 0,
        soft_objects: QUOTA_UNLIMITED,
        hard_objects: QUOTA_UNLIMITED,
        flags: 0,
        _pad: 0,
    })
}
...
pub fn set_quota(set: &QuotaSetArgs) -> ExofsResult<()> {
    ...
    if !found {
        if entries.len() >= QUOTA_MAX_ENTRIES {
...
pub fn sys_exofs_quota_query(
    ...
    if args.flags & quota_flags::SET != 0 {
        ...
        if let Err(e) = set_quota(&set_args) {
            return exofs_err_to_errno(e);
        }
        return 0;
    }
...
mod tests {
    ...
    #[test]
    fn test_quota_args_size() {
        assert_eq!(core::mem::size_of::<QuotaQueryArgs>(), 48);
    }

    #[test]
    fn test_quota_info_size() {
        assert_eq!(core::mem::size_of::<QuotaInfo>(), 56);
    }

    #[test]
    fn test_quota_set_args_size() {
        assert_eq!(core::mem::size_of::<QuotaSetArgs>(), 48);
    }
    ...
    #[test]
    fn test_quota_magic() {
        assert_eq!(QUOTA_MAGIC, 0x5155_4F54);
    }

    #[test]
    fn test_quota_unlimited_const() {
        assert_eq!(QUOTA_UNLIMITED, u64::MAX);
    }

    #[test]
    fn test_quota_entry_size() {
        assert_eq!(QUOTA_ENTRY_SIZE, 64);
    }
...
pub fn quota_bytes_percent(owner_uid: u64) -> ExofsResult<u32> {
    let q = get_quota(owner_uid)?;
    if q.hard_bytes == 0 || q.hard_bytes == QUOTA_UNLIMITED {
        return Ok(0);
    }
    let pct = q
        .used_bytes
        .saturating_mul(100)
        .checked_div(q.hard_bytes)
        .unwrap_or(100);
