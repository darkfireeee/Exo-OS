...
impl ObjectKind {
    ...
    #[inline]
    pub fn is_directory(self) -> bool { matches!(self, Self::PathIndex) }

    /// Retourne vrai si le BlobId ne doit JAMAIS être exposé (règle SEC-07).
    #[inline]
    pub fn is_secret(self) -> bool { matches!(self, Self::Secret) }

    /// Retourne vrai si l'objet exige une validation avant exécution.
    #[inline]
    pub fn requires_exec_validation(self) -> bool { matches!(self, Self::Code) }

    /// Retourne vrai si l'objet exige une validation de schéma avant rechargement.
    #[inline]
    pub fn requires_schema_validation(self) -> bool { matches!(self, Self::Config) }

    /// Retourne vrai si l'objet est une relation (immuable).
    #[inline]
    pub fn is_relation(self) -> bool { matches!(self, Self::Relation) }
    ...
    pub fn check_kind_operation(self, op: KindOperation) -> Result<(), ExofsError> {
        ...
        if ok { Ok(()) } else { Err(ExofsError::WrongObjectKind) }
    }
...
impl ObjectKind {
    ...
    pub fn can_upgrade_to(self, target: Self) -> bool {
        matches!((self, target),
            (ObjectKind::Blob, ObjectKind::Config)
            | (ObjectKind::Config, ObjectKind::Blob)
        )
    }
...
pub fn minimum_rights_for_kind_op(
    kind: ObjectKind,
    op:   KindOperation,
) -> u32 {
