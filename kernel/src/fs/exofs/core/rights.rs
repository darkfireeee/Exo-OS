...
impl RightsMask {
    ...
    pub const READ_WRITE: Self = Self(
        RIGHT_READ | RIGHT_WRITE | RIGHT_CREATE | RIGHT_DELETE
        | RIGHT_STAT | RIGHT_SETMETA | RIGHT_LIST
    );
    ...
    #[inline]
    pub fn has(self, right: u32) -> bool { self.0 & right == right }

    /// Vrai si ce masque contient TOUS les droits de `other`.
    #[inline]
    pub fn contains_all(self, other: RightsMask) -> bool { self.0 & other.0 == other.0 }

    /// Vrai si ce masque contient AU MOINS UN droit de `other`.
    #[inline]
    pub fn contains_any(self, other: RightsMask) -> bool { self.0 & other.0 != 0 }
    ...
    #[inline]
    pub fn add(&mut self, right: u32) { self.0 |= right & ALL_RIGHTS; }

    /// Retire un droit du masque.
    #[inline]
    pub fn revoke(&mut self, right: u32) { self.0 &= !right; }
    ...
    #[inline]
    pub fn is_unprivileged(self) -> bool { self.0 & PRIVILEGED_RIGHTS == 0 }
...
impl core::ops::BitAnd for RightsMask {
    type Output = Self;
    fn bitand(self, rhs: Self) -> Self { self.intersect(rhs) }
}
...
#[inline] pub fn has_inspect_content(bits: u32)  -> bool { bits & RIGHT_INSPECT_CONTENT != 0 }
/// Vérifie si un bitmask inclut SNAPSHOT_CREATE.
#[inline] pub fn has_snapshot_create(bits: u32)  -> bool { bits & RIGHT_SNAPSHOT_CREATE  != 0 }
/// Vérifie si un bitmask inclut RELATION_CREATE.
#[inline] pub fn has_relation_create(bits: u32)  -> bool { bits & RIGHT_RELATION_CREATE  != 0 }
/// Vérifie si un bitmask inclut GC_TRIGGER.
#[inline] pub fn has_gc_trigger(bits: u32)        -> bool { bits & RIGHT_GC_TRIGGER       != 0 }
/// Vérifie si un bitmask inclut EXPORT.
#[inline] pub fn has_export(bits: u32)             -> bool { bits & RIGHT_EXPORT            != 0 }
/// Vérifie si un bitmask inclut IMPORT.
#[inline] pub fn has_import(bits: u32)             -> bool { bits & RIGHT_IMPORT            != 0 }
/// Vérifie si un bitmask inclut WRITE.
#[inline] pub fn has_write(bits: u32)              -> bool { bits & RIGHT_WRITE              != 0 }
/// Vérifie si un bitmask inclut READ.
#[inline] pub fn has_read(bits: u32)               -> bool { bits & RIGHT_READ               != 0 }
/// Vérifie si un bitmask inclut EXEC.
#[inline] pub fn has_exec(bits: u32)               -> bool { bits & RIGHT_EXEC               != 0 }
/// Vérifie si un bitmask inclut CREATE.
#[inline] pub fn has_create(bits: u32)             -> bool { bits & RIGHT_CREATE             != 0 }
/// Vérifie si un bitmask inclut DELETE.
#[inline] pub fn has_delete(bits: u32)             -> bool { bits & RIGHT_DELETE             != 0 }
...
impl RightsValidator {
    ...
    pub fn new(granted: RightsMask, policy: RightsPolicy) -> Self {
        Self { granted, policy, ok_count: 0, ko_count: 0 }
    }
...
pub fn inherit_rights(
    parent_rights: RightsMask,
    allowed_mask:  RightsMask,
) -> RightsMask {
...
impl RightsAuditEntry {
    ...
    pub fn approved(epoch: u64, requested: u32, granted: u32, kind: u8) -> Self {
        Self { epoch_id: epoch, requested, granted, allowed: 1, object_kind: kind, _reserved: [0; 2] }
    }
    /// Crée une entrée d'audit refusée.
    pub fn denied(epoch: u64, requested: u32, granted: u32, kind: u8) -> Self {
        Self { epoch_id: epoch, requested, granted, allowed: 0, object_kind: kind, _reserved: [0; 2] }
    }
    /// Vrai si la décision était un refus.
    pub fn is_denied(self) -> bool { self.allowed == 0 }
}
