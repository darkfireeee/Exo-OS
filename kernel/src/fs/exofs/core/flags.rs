...
use core::fmt;
use core::ops::{BitOr, BitAnd, BitOrAssign, BitAndAssign, Not};
...
macro_rules! impl_flags_common {
    ($T:ty, $Inner:ty) => {
        impl $T {
            ...
            #[inline]
            pub fn contains_any(self, flag: Self) -> bool { self.0 & flag.0 != 0 }
            ...
            #[inline]
            pub fn bits(self) -> $Inner { self.0 }
            /// Crée depuis bits bruts (pas de validation).
            #[inline]
            pub fn from_bits_unchecked(bits: $Inner) -> Self { Self(bits) }
        }
    };
}
...
impl Not for ObjectFlags {
    type Output = Self;
    fn not(self) -> Self { Self(!self.0 & Self::ALL_KNOWN.0) }
}
...
impl ObjectFlagsBuilder {
    pub fn new() -> Self { Self(ObjectFlags::NONE) }
    pub fn deleted(mut self)    -> Self { self.0 |= ObjectFlags::DELETED;    self }
    pub fn inline_data(mut self) -> Self { self.0 |= ObjectFlags::INLINE_DATA; self }
    pub fn encrypted(mut self)  -> Self { self.0 |= ObjectFlags::ENCRYPTED;  self }
    pub fn compressed(mut self) -> Self { self.0 |= ObjectFlags::COMPRESSED; self }
    pub fn deduped(mut self)    -> Self { self.0 |= ObjectFlags::DEDUPED;    self }
    pub fn snapshot(mut self)   -> Self { self.0 |= ObjectFlags::SNAPSHOT;   self }
    pub fn immutable(mut self)  -> Self { self.0 |= ObjectFlags::IMMUTABLE;  self }
    pub fn verified(mut self)   -> Self { self.0 |= ObjectFlags::VERIFIED;   self }
    ...
    pub fn build_unchecked(self) -> ObjectFlags { self.0 }
}
...
impl ExtentFlags {
    ...
    #[inline]
    pub fn is_virtual(self) -> bool { self.contains(Self::SPARSE) }
}
...
impl MigrationFlags {
    ...
    pub fn is_in_progress(self) -> bool { self.contains(Self::IN_PROGRESS) }
    pub fn is_done(self)        -> bool { self.contains(Self::COMPLETED) | self.contains(Self::ABORTED) }
}
...
pub fn validate_object_epoch_flags(
    obj_flags:   ObjectFlags,
    epoch_flags: EpochFlags,
) -> bool {
    // Un objet immuable ne peut pas avoir un CoW en attente.
    if obj_flags.0 & ObjectFlags::IMMUTABLE.0 != 0
        && obj_flags.0 & ObjectFlags::COW_PENDING.0 != 0
    {
        return false;
    }
...
pub fn snapshot_eligible(obj: ObjectFlags, snap: SnapshotFlags) -> bool {
    obj.0 & ObjectFlags::DELETED.0 == 0
        && snap.0 & SnapshotFlags::PENDING_DELETE.0 == 0
}
