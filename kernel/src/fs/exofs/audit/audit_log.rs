...
impl AuditLog {
    ...
    pub fn next_seq(&self) -> u64 { self.head.load(Ordering::Relaxed) }
    ...
    pub fn n_security(&self) -> u64 { self.n_security.load(Ordering::Relaxed) }

    /// Nombre d'entrées critiques ou alertes.
    pub fn n_critical(&self) -> u64 { self.n_critical.load(Ordering::Relaxed) }

    /// Nombre d'entrées en erreur (Error ou Timeout).
    pub fn n_errors(&self) -> u64 { self.n_errors.load(Ordering::Relaxed) }
...
impl AuditLogStats {
    ...
    pub fn is_near_full(&self) -> bool { self.ring_fill_pct >= 80 }
}
...
impl AuditLogHealth {
    ...
    pub fn is_clean(&self) -> bool { self.corrupt == 0 }
}
...
mod tests {
    ...
    #[test] fn test_initial_count_zero() {
        let log = AuditLog::new_const();
        assert_eq!(log.count(), 0);
        assert_eq!(log.available(), 0);
    }

    #[test] fn test_push_increments_count() {
        let log = AuditLog::new_const();
    ...
    #[test] fn test_read_last_none_when_empty() {
        let log = AuditLog::new_const();
        assert!(log.read_last().is_none());
    }

    #[test] fn test_read_last_returns_last_pushed() {
        let log = AuditLog::new_const();
    ...
    #[test] fn test_stats_near_full_false_on_fresh() {
        let log  = AuditLog::new_const();
        let stat = log.stats();
    ...
    #[test] fn test_read_recent_into_zero() {
        let log = AuditLog::new_const();
        let mut buf = [];
    ...
    #[test] fn test_for_each_no_panic_empty() {
        let log = AuditLog::new_const();
        let mut count = 0usize;
        log.for_each(|_| count += 1);
        assert_eq!(count, 0);
