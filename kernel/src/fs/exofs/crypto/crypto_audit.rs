...
mod tests {
    ...
    fn fresh() -> CryptoAuditLog {
        CryptoAuditLog::new_const()
    }
    ...
    #[test]
    fn test_total_written_increments() {
        let log = fresh();
        for _ in 0..5 {
    ...
    pub fn summary(&self) -> ExofsResult<AuditSummary> {
        ...
        for e in &entries {
            ...
            match e.kind {
                ...
                | AuditKind::KeyRotated
                | AuditKind::KeyWrapped
                | AuditKind::KeyUnwrapped => s.key_ops = s.key_ops.saturating_add(1),
                AuditKind::BlobEncrypted | AuditKind::BlobDecrypted | AuditKind::BlobShredded => {
                    s.blob_ops = s.blob_ops.saturating_add(1)
                }
                AuditKind::AuthFailure => s.auth_failures = s.auth_failures.saturating_add(1),
            }
        }
        Ok(s)
    }
...
mod extended_audit_tests {
    ...
    fn fl() -> CryptoAuditLog {
        CryptoAuditLog::new_const()
    }

    #[test]
    fn test_summary_counts_key_ops() {
        let l = fl();
    ...
    #[test]
    fn test_range_invalid_order_fails() {
        let l = fl();
        assert!(l.range(10, 5).is_err());
    }

    #[test]
    fn test_concurrent_record_no_panic() {
        let l = fl();
        for i in 0..50u64 {
            l.record(AuditKind::BlobShredded, Some(KeySlotId(i)), i, i % 2 == 0);
        }
