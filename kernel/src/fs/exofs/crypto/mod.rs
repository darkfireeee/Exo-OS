...
pub use xchacha20::{Nonce, Tag, XChaCha20Key, XChaCha20Poly1305};

// Entropie
pub use entropy::{generate_salt, generate_unique_id, EntropyPool, ENTROPY_POOL};

// Dérivation de clés
pub use key_derivation::{DerivedKey, KeyDerivation, KeyPurpose};

// Clé maître
pub use master_key::{MasterKey, MasterKeyId, WrappedMasterKey};

// Clé volume
pub use volume_key::{ObjectKeyCache, VolumeId, VolumeKey, WrappedVolumeKey};

// Clé objet / blob
pub use object_key::{BlobKeyId, ObjectKey, ObjectKeyPool, RotationPolicy};

// Stockage de clés
pub use key_storage::{KeyKind, KeySlotId, KeyStorage, SlotInfo, SlotState, KEY_STORAGE};

// Rotation de clés
pub use key_rotation::{KeyRotation, RotationReason, RotationResult, RotationSchedule};

// Audit
pub use crypto_audit::{AuditEntry, AuditKind, AuditSummary, CryptoAuditLog, AUDIT_LOG};

// Destruction sécurisée
pub use crypto_shredding::{
    CryptoShredder, NullOverwriter, OverwriteBlob, OverwriteStrategy, ShredPolicy, ShredResult,
    ShredScheduler,
};

// Lecture / écriture de secrets
pub use secret_reader::{ReadResult, SecretHeader, SecretReader, SECRET_HEADER_SIZE, SECRET_MAGIC};
pub use secret_writer::{
    build_payload, EncryptedBlob, EnvelopeWriter, SealedEnvelope, SecretWriteResult, SecretWriter,
    SecretWriterPool, WriterId,
};
...
impl CryptoModule {
    ...
    pub fn shred_blob<O: OverwriteBlob>(
        ...
        self.shredder
            .shred_blob(blob_id, blob_size, slot_id, Some(&KEY_STORAGE), writer)
    }
    ...
    pub fn audit_summary(&self) -> ExofsResult<AuditSummary> {
        AUDIT_LOG.summary()
    }
}
...
mod tests {
    ...
    #[test]
    fn test_crypto_config_default_valid() {
        let cfg = CryptoConfig::default();
        assert!(cfg.validate().is_ok());
    }

    #[test]
    fn test_crypto_config_minimal_valid() {
        let cfg = CryptoConfig::minimal();
        assert!(cfg.validate().is_ok());
    }

    #[test]
    fn test_crypto_config_invalid_cache_size() {
    ...
    #[test]
    fn test_crypto_config_invalid_max_keys() {
        let mut cfg = CryptoConfig::default();
    ...
    #[test]
    fn test_secret_magic_constant() {
        assert_eq!(SECRET_MAGIC, [0xEF, 0x5E, 0x52, 0x44]);
    }

    #[test]
    fn test_secret_header_size_constant() {
        assert_eq!(SECRET_HEADER_SIZE, 52);
    }

    #[test]
    fn test_xchacha_key_zeroize() {
    ...
    #[test]
    fn test_entropy_pool_accessible() {
...
impl CryptoStats {
    ...
    pub fn record_shred(&self) {
        self.shred_ops.fetch_add(1, Ordering::Relaxed);
    }
    pub fn record_derive(&self) {
        self.derive_ops.fetch_add(1, Ordering::Relaxed);
    }
    ...
    pub fn encrypt_count(&self) -> u64 {
        self.encrypt_ops.load(Ordering::Relaxed)
    }
    pub fn decrypt_count(&self) -> u64 {
        self.decrypt_ops.load(Ordering::Relaxed)
    }
    pub fn shred_count(&self) -> u64 {
        self.shred_ops.load(Ordering::Relaxed)
    }
    pub fn derive_count(&self) -> u64 {
        self.derive_ops.load(Ordering::Relaxed)
    }
...
mod tests_meta {
    ...
    #[test]
    fn test_crypto_stats_increment() {
        CRYPTO_STATS.reset();
    ...
    #[test]
    fn test_crypto_stats_auth_error() {
        CRYPTO_STATS.reset();
