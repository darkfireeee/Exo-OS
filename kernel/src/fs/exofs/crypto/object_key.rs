...
impl core::fmt::Debug for ObjectKey {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "ObjectKey {{ blob_id: {:?}, use_count: {}, revoked: {} }}",
...
impl ObjectKey {
    ...
    pub fn derive(vk: &VolumeKey, blob_id: BlobKeyId) -> ExofsResult<Self> {
        ...
        Ok(Self { key: raw, blob_id, use_count: 0, revoked: false })
    }
    ...
    pub fn generate_ephemeral(blob_id: BlobKeyId) -> ExofsResult<Self> {
        ...
        Ok(Self { key, blob_id, use_count: 0, revoked: false })
    }

    /// Construit depuis des bytes bruts (import sécurisé).
    pub fn from_bytes(bytes: [u8; OBJECT_KEY_LEN], blob_id: BlobKeyId) -> Self {
        Self { key: bytes, blob_id, use_count: 0, revoked: false }
    }

    // ── Accesseurs ────────────────────────────────────────────────────────────

    pub fn blob_id(&self) -> BlobKeyId { self.blob_id }
    pub fn use_count(&self) -> u64     { self.use_count }
    pub fn is_revoked(&self) -> bool   { self.revoked }

    /// Retourne les bytes bruts (durée de vie très courte — ne pas stocker).
    pub fn raw_bytes(&self) -> &[u8; OBJECT_KEY_LEN] { &self.key }
    ...
    pub fn record_use(&mut self) -> ExofsResult<()> {
        if self.revoked { return Err(ExofsError::InternalError); }
...
impl ObjectKeyPool {
    /// Crée un pool vide.
    pub fn new() -> Self {
        Self { active: Vec::new(), revoked: Vec::new() }
    }
    ...
    pub fn insert(&mut self, key: ObjectKey) -> ExofsResult<()> {
        self.active.try_reserve(1).map_err(|_| ExofsError::NoMemory)?;
    ...
    pub fn revoke(&mut self, blob_id: BlobKeyId) -> ExofsResult<()> {
        ...
        if !self.revoked.contains(&blob_id) {
            self.revoked.try_reserve(1).map_err(|_| ExofsError::NoMemory)?;
...
mod tests {
    use super::*;
    use super::super::volume_key::{VolumeKey, VolumeId};

    fn vk() -> VolumeKey { VolumeKey::generate(VolumeId(1)).unwrap() }

    #[test] fn test_derive_ok() {
        let vk = vk();
        let ok = ObjectKey::derive(&vk, BlobKeyId(1)).unwrap();
        assert_eq!(ok.raw_bytes().len(), 32);
    }

    #[test] fn test_different_blobs_different_keys() {
        let vk  = vk();
    ...
    #[test] fn test_record_use_after_revoke_fails() {
        let vk  = vk();
        let mut ok = ObjectKey::derive(&vk, BlobKeyId(1)).unwrap();
        ok.revoke();
    ...
    #[test] fn test_subkey_integrity_different() {
        let vk = vk();
        let ok = ObjectKey::derive(&vk, BlobKeyId(1)).unwrap();
        let dk = ok.data_key().unwrap();
    ...
    #[test] fn test_pool_insert_revoke() {
        let vk  = vk();
...
impl ObjectKeyRotator {
    ...
    pub fn new(policy: RotationPolicy) -> Self {
        Self { policy, rotated: Vec::new(), pending: Vec::new() }
    }
    ...
    pub fn mark_rotated(&mut self, blob_id: BlobKeyId) -> ExofsResult<()> {
        ...
        if !self.rotated.contains(&blob_id) {
            self.rotated.try_reserve(1).map_err(|_| ExofsError::NoMemory)?;
    ...
    pub fn pending_rotation(&self) -> &[BlobKeyId] { &self.pending }
    /// Liste des blob_ids déjà tournés.
    pub fn rotated_ids(&self) -> &[BlobKeyId]      { &self.rotated }
...
mod rotation_tests {
    ...
    fn vk() -> VolumeKey { VolumeKey::generate(VolumeId(2)).unwrap() }
    ...
    #[test] fn test_policy_after_n() {
        let vk = vk();
        let mut ok = ObjectKey::derive(&vk, BlobKeyId(1)).unwrap();
        for _ in 0..4 { ok.record_use().unwrap(); }
        assert!(!RotationPolicy::AfterNUses(5).should_rotate(&ok));
        ok.record_use().unwrap();
        assert!(RotationPolicy::AfterNUses(5).should_rotate(&ok));
    }

    #[test] fn test_rotator_evaluate_marks_pending() {
        let vk = vk();
    ...
    #[test] fn test_rotator_mark_rotated() {
        let vk = vk();
        let mut ok = ObjectKey::derive(&vk, BlobKeyId(10)).unwrap();
        ok.record_use().unwrap();
        let mut rot = ObjectKeyRotator::new(RotationPolicy::Once);
