...
impl core::fmt::Debug for VolumeKey {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "VolumeKey {{ volume_id: {:?}, key: <redacted> }}", self.volume_id)
    }
}
...
impl WrappedVolumeKey {
    ...
    pub fn from_bytes(bytes: &[u8; WRAPPED_VK_SIZE]) -> ExofsResult<Self> {
        let magic = u32::from_le_bytes(bytes[..4].try_into().map_err(|_| ExofsError::CorruptedStructure)?);
        if magic != VOLUME_KEY_MAGIC { return Err(ExofsError::InvalidMagic); }
        let volume_id = VolumeId(u64::from_le_bytes(bytes[4..12].try_into().map_err(|_| ExofsError::CorruptedStructure)?));
        let mut salt  = [0u8; 32]; salt.copy_from_slice(&bytes[12..44]);
        let mut ct    = [0u8; 32]; ct.copy_from_slice(&bytes[44..76]);
        let mut mac   = [0u8; 32]; mac.copy_from_slice(&bytes[76..108]);
        Ok(Self { magic, volume_id, salt, ct, mac })
    }
}
...
impl VolumeKey {
    ...
    pub fn derive_from_master(master: &MasterKey, volume_id: VolumeId) -> ExofsResult<Self> {
        ...
        Ok(Self { key: raw, volume_id })
    }

    /// Construit depuis des bytes bruts (import).
    pub fn from_bytes(bytes: [u8; VOLUME_KEY_LEN], volume_id: VolumeId) -> Self {
        Self { key: bytes, volume_id }
    }

    // ── Accesseurs ────────────────────────────────────────────────────────────

    pub fn volume_id(&self) -> VolumeId { self.volume_id }
    pub fn raw_bytes(&self) -> &[u8; VOLUME_KEY_LEN] { &self.key }
    ...
    pub fn wrap(&self, master: &MasterKey) -> ExofsResult<WrappedVolumeKey> {
        ...
        let mut salt = [0u8; 32]; salt.copy_from_slice(&salt_raw);
        ...
        for i in 0..VOLUME_KEY_LEN { ct[i] = self.key[i] ^ kek[i]; }

        // MAC.
        let mac = vk_mac(VOLUME_KEY_MAGIC, self.volume_id, &salt, &ct, kek);
        Ok(WrappedVolumeKey { magic: VOLUME_KEY_MAGIC, volume_id: self.volume_id, salt, ct, mac })
    }

    /// Déencapsule une clé de volume avec la clé maître.
    pub fn unwrap(wrapped: &WrappedVolumeKey, master: &MasterKey) -> ExofsResult<Self> {
        if wrapped.magic != VOLUME_KEY_MAGIC { return Err(ExofsError::InvalidMagic); }
        let kek_dk = KeyDerivation::derive_key(master.raw_bytes(), &wrapped.salt, b"exofs-wrap-vk")?;
        let kek    = kek_dk.as_bytes();
        let expected = vk_mac(wrapped.magic, wrapped.volume_id, &wrapped.salt, &wrapped.ct, kek);
        if !ct_eq_32(&expected, &wrapped.mac) { return Err(ExofsError::CorruptedStructure); }
        let mut key = [0u8; VOLUME_KEY_LEN];
        for i in 0..VOLUME_KEY_LEN { key[i] = wrapped.ct[i] ^ kek[i]; }
        Ok(Self { key, volume_id: wrapped.volume_id })
    }
}
...
fn vk_mac(magic: u32, vid: VolumeId, salt: &[u8;32], ct: &[u8;32], mk: &[u8;32]) -> [u8;32] {
    ...
    d.extend_from_slice(salt); d.extend_from_slice(ct);
    hmac_sha256_simple(mk, &d)
}

fn hmac_sha256_simple(key: &[u8], msg: &[u8]) -> [u8; 32] {
    let mut k = [0u8; 64];
    if key.len() > 64 { let h = sha256_simple(key); k[..32].copy_from_slice(&h); }
    else { k[..key.len()].copy_from_slice(key); }
    let mut ip = [0x36u8;64]; let mut op = [0x5cu8;64];
    for i in 0..64 { ip[i]^=k[i]; op[i]^=k[i]; }
    let mut inn = Vec::new(); let _ = inn.try_reserve(64+msg.len());
    inn.extend_from_slice(&ip); inn.extend_from_slice(msg);
    let ih = sha256_simple(&inn);
    let mut out2 = Vec::new(); let _ = out2.try_reserve(96);
    out2.extend_from_slice(&op); out2.extend_from_slice(&ih);
    sha256_simple(&out2)
}
...
fn sha256_simple(msg: &[u8]) -> [u8;32] {
    ...
    for ch in p.chunks_exact(64) {
        let mut w=[0u32;64];
        for i in 0..16{ w[i]=u32::from_be_bytes(ch[i*4..i*4+4].try_into().unwrap_or([0;4])); }
        for i in 16..64{
        ...
        for i in 0..64{
            ...
            let ch2=(e&f)^((!e)&g);
            let t1=hh.wrapping_add(s1).wrapping_add(ch2).wrapping_add(K[i]).wrapping_add(w[i]);
...
fn ct_eq_32(a:&[u8;32],b:&[u8;32])->bool{ let mut d=0u8; for i in 0..32{d|=a[i]^b[i];} d==0 }
...
mod tests {
    ...
    #[test] fn test_derive_from_master_ok() {
        let mk = mk();
        let vk = VolumeKey::derive_from_master(&mk, VolumeId(42)).unwrap();
        assert_eq!(vk.raw_bytes().len(), 32);
    }

    #[test] fn test_derive_from_master_different_vols() {
        let mk  = mk();
    ...
    #[test] fn test_from_bytes_ok() {
        let b  = [0x42u8; 32];
        let vk = VolumeKey::from_bytes(b, VolumeId(5));
        assert_eq!(*vk.raw_bytes(), b);
    }

    #[test] fn test_derive_object_key_ok() {
        let vk = VolumeKey::generate(VolumeId(1)).unwrap();
        let ok = vk.derive_object_key(99).unwrap();
        assert_eq!(ok.len(), 32);
    }

    #[test] fn test_derive_object_key_different_blobs() {
        let vk  = VolumeKey::generate(VolumeId(1)).unwrap();
...
struct CacheEntry { blob_id: u64, key: [u8; 32] }
...
impl ObjectKeyCache {
    ...
    pub fn new(cap: usize) -> ExofsResult<Self> {
        ...
        if cap > 0 { v.try_reserve(cap).map_err(|_| ExofsError::NoMemory)?; }
        Ok(Self { entries: v, capacity: cap })
    }
    ...
    pub fn get_or_derive(&mut self, vk: &VolumeKey, blob_id: u64) -> ExofsResult<[u8; 32]> {
        ...
        for e in &self.entries {
            if e.blob_id == blob_id { return Ok(e.key); }
        }
        // Dérivation.
...
mod cache_tests {
    ...
    #[test] fn test_cache_basic() {
        ...
        assert_eq!(k1, k2); assert_eq!(c.len(), 1);
    }
    ...
    #[test] fn test_cache_invalidate() {
        let vk  = VolumeKey::generate(VolumeId(1)).unwrap();
        let mut c = ObjectKeyCache::new(4).unwrap();
        c.get_or_derive(&vk, 5).unwrap();
        c.invalidate(5); assert!(c.is_empty());
    }
    ...
    #[test] fn test_cache_clear() {
        ...
        c.get_or_derive(&vk, 1).unwrap();
        c.get_or_derive(&vk, 2).unwrap();
        c.clear(); assert!(c.is_empty());
    }

    #[test] fn test_cache_zero_capacity_no_cache() {
        let vk  = VolumeKey::generate(VolumeId(1)).unwrap();
