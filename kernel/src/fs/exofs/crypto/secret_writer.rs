...
impl EncryptedBlob {
    ...
    pub fn ciphertext_len(&self) -> usize {
        self.raw.len().saturating_sub(SECRET_HEADER_SIZE)
    }
}
...
impl SecretWriterPool {
    ...
    pub fn remove(&mut self, id: WriterId) {
...
mod tests {
    ...
    #[test]
    fn test_encrypt_empty_data_fails() {
        assert_eq!(
            writer().encrypt(b"").unwrap_err(),
    ...
    #[test]
    fn test_payload_size_for_overflow() {
        assert!(payload_size_for(usize::MAX).is_err());
    }

    #[test]
    fn test_is_valid_payload_size() {
...
impl EnvelopeWriter {
    ...
    pub fn new(raw_key: &[u8; 32]) -> Self {
        Self {
            writer: SecretWriter::new(raw_key),
            next_env: 1,
        }
    }
...
mod tests_envelope {
    ...
    #[test]
    fn test_seal_empty_plaintext_fails() {
        let mut w = EnvelopeWriter::new(&key32());
