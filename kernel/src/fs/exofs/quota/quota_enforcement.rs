...
impl EnforcementAction {
    pub fn is_deny(self)    -> bool { matches!(self, Self::Deny | Self::DenyGrace) }
    pub fn is_allow(self)   -> bool { !self.is_deny() }
...
impl EnforcementResult {
    pub fn allow(key: QuotaKey) -> Self {
        Self {
            ...
            used_bytes:   0, used_blobs:   0, used_inodes:   0,
            limit_bytes:  0, limit_blobs:  0, limit_inodes:  0,
            grace_ticks:  0, requested:    0,
        }
    }

    pub fn is_allowed(self)  -> bool  { self.action.is_allow() }
    pub fn is_denied(self)   -> bool  { self.action.is_deny() }

    /// Fraction utilisée en ‰ (bytes), ARITH-02.
    pub fn bytes_ppt(self) -> u64 {
        if self.limit_bytes == 0 || self.limit_bytes == u64::MAX { return 0; }
        self.used_bytes.saturating_mul(1000)
            .checked_div(self.limit_bytes).unwrap_or(1000)
    }

    /// Fraction utilisée en ‰ (inodes), ARITH-02.
    pub fn inodes_ppt(self) -> u64 {
        if self.limit_inodes == 0 || self.limit_inodes == u64::MAX { return 0; }
        self.used_inodes.saturating_mul(1000)
            .checked_div(self.limit_inodes).unwrap_or(1000)
    }

    pub fn reason_str(self) -> &'static str { self.action.reason_str() }
}
...
impl QuotaViolation {
    pub fn new(
        ...
        Self { key, event, bytes_requested, current_bytes, limit_bytes, tick }
    }
...
impl EnforcementStats {
    pub const fn zero() -> Self {
        Self { checks_total: 0, allowed: 0, soft_breaches: 0, hard_denials: 0, grace_denials: 0 }
    }
    pub fn denial_rate_ppt(&self) -> u64 {
        if self.checks_total == 0 { return 0; }
        let denials = self.hard_denials.saturating_add(self.grace_denials);
        denials.saturating_mul(1000).checked_div(self.checks_total).unwrap_or(0)
    }
    pub fn is_healthy(&self) -> bool { self.denial_rate_ppt() < 50 }
}
...
impl QuotaEnforcer {
    ...
    pub fn check_bytes(&self, key: QuotaKey, request_bytes: u64, tick: u64)
        -> ExofsResult<EnforcementResult>
    {
        ...
        let used = QUOTA_TRACKER.get_usage(key)
            .map(|u| u.bytes_used).unwrap_or(0);
    ...
    /// Vérifie les blobs.
    pub fn check_blobs(&self, key: QuotaKey, request_blobs: u64, tick: u64)
        -> ExofsResult<EnforcementResult>
    {
        let limits = match QUOTA_TRACKER.get_limits(key) {
            Some(l) if !l.is_unlimited() => l,
        ...
        let used = QUOTA_TRACKER.get_usage(key)
            .map(|u| u.blobs_used).unwrap_or(0);
    ...
    /// Vérifie les inodes.
    pub fn check_inodes(&self, key: QuotaKey, request_inodes: u64, tick: u64)
        -> ExofsResult<EnforcementResult>
    {
        let limits = match QUOTA_TRACKER.get_limits(key) {
            Some(l) if !l.is_unlimited() => l,
        ...
        let used = QUOTA_TRACKER.get_usage(key)
            .map(|u| u.inodes_used).unwrap_or(0);
    ...
    pub fn check_write(
        ...
        if rb.is_denied() { return Ok(rb); }
        let rbl = self.check_blobs(key, blobs, tick)?;
        if rbl.is_denied() { return Ok(rbl); }
    ...
    pub fn apply(&self, result: &EnforcementResult) -> ExofsResult<()> {
        if result.is_denied() { Err(ExofsError::QuotaExceeded) } else { Ok(()) }
    }
    ...
    fn _build(
        ...
        let limits = QUOTA_TRACKER.get_limits(key)
...
mod tests {
    ...
    fn test_apply_deny_returns_quota_exceeded() {
        ...
        let result = EnforcementResult {
            ...
            entity_id:    0, used_bytes: 0, used_blobs: 0, used_inodes: 0,
            limit_bytes:  0, limit_blobs: 0, limit_inodes: 0,
            grace_ticks:  0, requested:  0,
        };
        assert!(matches!(enforcer.apply(&result), Err(ExofsError::QuotaExceeded)));
    }
