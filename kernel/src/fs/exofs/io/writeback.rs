...
impl WritebackEntry {
    ...
    pub fn age_ticks(&self, now: u64) -> u64 {
        now.saturating_sub(self.dirty_since)
    }
...
impl WritebackConfig {
    pub fn default() -> Self {
        Self {
            max_queue_depth: 256,
            max_retries: 5,
            dirty_age_us_threshold: 5_000_000,
            flush_batch_size: 32,
    ...
    pub fn conservative() -> Self {
        Self {
            max_queue_depth: 64,
            max_retries: 3,
            dirty_age_us_threshold: 1_000_000,
            flush_batch_size: 8,
...
impl WritebackQueue {
    ...
    pub fn is_full(&self) -> bool {
        self.pending.load(Ordering::Relaxed) >= WRITEBACK_QUEUE_DEPTH as u64
    }
    ...
    pub fn collect_urgent(
        &self,
        now: u64,
        threshold_us: u64,
        out: &mut Vec<WritebackEntry>,
    ) -> ExofsResult<u32> {
        self.acquire();
        let result = (|| {
...
impl WritebackWorker {
    ...
    pub fn process_one(
        ...
        write_fn: &mut dyn FnMut(&[u8; 32], u32) -> ExofsResult<u64>,
    ) -> ExofsResult<bool> {
...
mod tests_helpers {
    ...
    fn test_dedup_entries() {
        ...
        let has_id1 = entries
            .iter()
            .any(|e| e.blob_id[0] == 1 && e.dirty_since == 20);
        assert!(has_id1);
    }
}
