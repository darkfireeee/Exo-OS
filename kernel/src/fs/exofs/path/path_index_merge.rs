...
impl PathIndexMerger {
    ...
    pub fn merge_many(indices: &[&PathIndex], parent_oid: ObjectId) -> ExofsResult<MergeResult> {
        if indices.is_empty() {
        ...
        if indices.len() == 1 {
            ...
            for e in indices[0].entries() {
                let comp =
                    validate_component(e.name_bytes()).map_err(|_| ExofsError::InternalError)?;
        ...
        for idx in indices {
            ...
            for e in idx.entries() {
                ...
                match acc.insert(&comp, e.oid.clone(), e.kind) {
                    Ok(()) => {}
                    Err(ExofsError::ObjectAlreadyExists) => {
                        total_reclaimed = total_reclaimed.saturating_add(60);
                    }
                    Err(err) => return Err(err),
                }
            }
    ...
    pub fn try_merge(
        ...
        if low.is_empty() && high.is_empty() {
            return Err(MergeRefusal::BothEmpty);
        }
...
pub fn merge_with_policy(
    ...
    for e in second.entries() {
        ...
        match merged.insert(&comp, e.oid.clone(), e.kind) {
            Ok(()) => {}
            Err(ExofsError::ObjectAlreadyExists) => {
                if policy == MergeConflictPolicy::Reject {
...
impl MergeRequest {
    pub fn new(parent_oid: ObjectId) -> Self {
        MergeRequest {
            parent_oid,
            policy: MergeConflictPolicy::PreferLow,
            verify: true,
        }
    }

    pub fn with_policy(mut self, p: MergeConflictPolicy) -> Self {
        self.policy = p;
        self
    }
...
impl MergeValidator {
    ...
    pub fn verify(result: &MergeResult, low: &PathIndex, high: &PathIndex) -> ExofsResult<()> {
        // Vérifier count cohérent.
        let max_expected = low
            .len()
            .checked_add(high.len())
            .ok_or(ExofsError::OffsetOverflow)?;
...
mod tests {
    ...
    #[test]
    fn test_merge_too_large() {
        let low = make_index(b'a', 100);
        let high = make_index(b'b', 100);
        assert!(matches!(
            PathIndexMerger::merge(&low, &high, fake_oid(0)),
