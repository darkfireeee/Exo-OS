...
use super::support::{
    close_fd, install_mock_disk, open_dir, open_path_atomic, open_rdwr, parse_dirents, read_at,
    readdir_fd, reset_state, write_at,
};
use crate::fs::exofs::cache::{BlobCache, BLOB_CACHE};
...
fn superblock_format_rejects_disk_too_small() {
    // format() retourne InvalidSize (pas DiskTooSmall) pour les disques trop petits.
    // Comportement intentionnel : format() utilise InvalidSize, mount() utilise DiskTooSmall.
    let result = SuperblockManager::format(
        4 * 1024 * 1024, // 4 MiB < 16 MiB minimum
        b"tiny",
        [0u8; 16],
        0,
        |_, buf| Ok(buf.len()),
    ...
    assert!(
        matches!(
            result,
            Err(ExofsError::InvalidSize) | Err(ExofsError::DiskTooSmall { .. })
        ),
        "format() doit rejeter un disque < MIN_DISK_SIZE, got: {result:?}"
    );
}
...
fn readdir_lists_created_entries() {
    ...
    // Ouvrir le répertoire avec kind=Directory pour que readdir fonctionne
    let dir_fd = open_dir("/std/dir/listing");
    ...
    assert!(
        ...
        "alpha.txt doit être listé, trouvés: {names:?}"
    );
...
fn readdir_empty_dir_returns_empty_or_dot_entries() {
    ...
    let dir_fd = open_dir("/std/dir/empty");
    let buf = readdir_fd(dir_fd, 4096);
    close_fd(dir_fd);
    let _ = parse_dirents(&buf); // doit retourner sans panique
    reset_state();
}
...
fn readdir_buffer_too_small_truncates_gracefully() {
    ...
    let dir_fd = open_dir("/std/dir/small");
    let buf = readdir_fd(dir_fd, HEADER_SIZE + 32);
    close_fd(dir_fd);
    let entries = parse_dirents(&buf);
    assert!(
        entries.len() <= 10,
        "ne peut pas dépasser le nombre de fichiers créés"
    );
    reset_state();
}
...
fn error_gc_delay_constant_is_unique() {
    ...
    #[allow(deprecated)]
    {
        ...
        assert_eq!(
            GC_MIN_EPOCH_DELAY, GC_MIN_EPOCH_DELAY_SECS,
            "les deux constantes doivent avoir la meme valeur"
        );
    }
}
