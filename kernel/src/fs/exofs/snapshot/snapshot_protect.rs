...
impl ProtectEntry {
    fn new(snap_id: SnapshotId, worm: Option<WormPolicy>, now: u64) -> Self {
        Self { snap_id, worm, protected_at: now, note: [0u8; 64] }
    }

    pub fn is_worm(&self) -> bool { self.worm.is_some() }
...
impl SnapshotProtect {
    ...
    pub fn protect_worm(&self, snap_id: SnapshotId, retain_ticks: u64, now: u64) -> ExofsResult<()> {
        let worm = WormPolicy { activated_at: now, retain_ticks };
        self.protect_inner(snap_id, Some(worm), now)
    }

    fn protect_inner(&self, snap_id: SnapshotId, worm: Option<WormPolicy>, now: u64) -> ExofsResult<()> {
        ...
        if is_worm { self.n_worm.fetch_add(1, Ordering::AcqRel); }
        Ok(())
    }
    ...
    pub fn unprotect(&self, snap_id: SnapshotId, now: u64) -> ExofsResult<()> {
        ...
        if was_worm { self.n_worm.fetch_sub(1, Ordering::AcqRel); }
        Ok(())
    }
    ...
    pub fn n_protected(&self) -> usize { self.n_protected.load(Ordering::Acquire) }
    pub fn n_worm(&self)      -> usize { self.n_worm.load(Ordering::Acquire) }
    ...
    pub fn stats(&self) -> ProtectStats {
        ...
        for e in guard.values() {
            match &e.worm {
                None => n_permanent += 1,
                Some(w) => if w.retain_ticks > 0 { n_expiring_soon += 1; },
            }
        }
        ProtectStats {
    ...
    pub fn set_note(&self, snap_id: SnapshotId, note: &[u8]) -> ExofsResult<()> {
        ...
        if len < 64 { entry.note[len..].fill(0); }
        Ok(())
    }
...
mod tests {
    ...
    fn expired_worm_list() {
        let list = SnapshotList::new_const();
        push_snap(&list, 10); push_snap(&list, 11);
        let p = SnapshotProtect::new_const();
    ...
    fn stats_count_correct() {
        let list = SnapshotList::new_const();
        push_snap(&list, 20); push_snap(&list, 21);
        let p = SnapshotProtect::new_const();
