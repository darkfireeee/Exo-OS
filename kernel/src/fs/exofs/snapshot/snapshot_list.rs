...
impl SnapshotList {
    ...
    pub fn find_by_name(&self, name: &[u8]) -> Option<SnapshotRef> {
        ...
        for snap in guard.values() {
            ...
            if &snap.name[..len] == &name[..len] && snap.name[len..].iter().all(|&b| b == 0) {
    ...
    where
        F: FnOnce(&mut Snapshot) -> ExofsResult<()>,
    {
        let mut guard = self.inner.lock();
    ...
    pub fn stats(&self) -> ListStats {
        ...
        for snap in guard.values() {
            ...
            if snap.created_at > s.newest_created_at {
                s.newest_created_at = snap.created_at;
            }
            if snap.flags & f::PROTECTED != 0 {
                s.n_protected += 1;
            }
            if snap.flags & f::READONLY != 0 {
                s.n_readonly += 1;
            }
            if snap.flags & f::ORPHAN != 0 {
                s.n_orphan += 1;
            }
            s.total_bytes = s.total_bytes.saturating_add(snap.total_bytes);
            if snap.n_blobs > s.max_blobs_in_one {
                s.max_blobs_in_one = snap.n_blobs;
            }
            if snap.total_bytes > s.max_bytes_in_one {
                s.max_bytes_in_one = snap.total_bytes;
            }
        }
        s
    }
    ...
    pub fn verify_consistency(&self) -> Result<(), ListConsistencyError> {
        ...
        if tc != actual_count {
            return Err(ListConsistencyError::CountMismatch {
                tracked: tc,
                actual: actual_count,
            });
        }
        if tb != actual_bytes {
            return Err(ListConsistencyError::BytesMismatch {
                tracked: tb,
                actual: actual_bytes,
            });
        }
