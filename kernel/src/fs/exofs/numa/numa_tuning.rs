...
impl TuningReport {
    pub fn strategy_changed(self) -> bool { self.event == TuningEvent::StrategyChanged }
    pub fn is_balanced(self) -> bool { self.imbalance_ppt < self.threshold_ppt }
}
...
impl PressureZone {
    pub fn from_ppt(usage_ppt: u64) -> Self {
        if usage_ppt >= 950      { Self::Emergency }
        else if usage_ppt >= 850 { Self::Critical }
        else if usage_ppt >= 700 { Self::Warning }
        else                     { Self::Normal }
    }
    pub fn name(self) -> &'static str {
        match self {
    ...
    pub fn is_ok(self) -> bool { matches!(self, Self::Normal) }
}
...
impl NumaPolicy {
    ...
    pub fn configure(
        ...
        if threshold_ppt > 1000 { return Err(ExofsError::InvalidArgument); }
        if interval_ticks == 0  { return Err(ExofsError::InvalidArgument); }
        self.migrate_threshold_ppt.store(threshold_ppt, Ordering::Relaxed);
        self.auto_tune_enabled.store(u8::from(auto), Ordering::Relaxed);
        self.tune_interval_ticks.store(interval_ticks, Ordering::Relaxed);
        Ok(())
    }

    /// Définit la capacité mémoire d'un nœud.
    pub fn set_node_capacity(&self, node: usize, capacity_bytes: u64) -> ExofsResult<()> {
        if node >= MAX_NUMA_NODES { return Err(ExofsError::InvalidArgument); }
    ...
    pub fn evaluate(&self, current_tick: u64) -> TuningReport {
        ...
        if !self.auto_enabled() {
            return self._make_report(TuningEvent::NoChange, 0, old_strategy, old_strategy, current_tick);
        }
        ...
        if forced != u8::MAX {
            ...
            return self._make_report(
                TuningEvent::ManualOverride, 0, old_strategy, new_strat, current_tick
            );
        }
    ...
    fn _compute_imbalance_ppt(&self) -> u64 {
        ...
        while i < MAX_NUMA_NODES {
            ...
            if capacity == 0 { i = i.wrapping_add(1); continue; }
            let stats  = NUMA_STATS.node_stats(i);
            let ppt    = stats.usage_ppt(capacity);
            if ppt > max_score { max_score = ppt; }
            if ppt < min_score { min_score = ppt; }
            i = i.wrapping_add(1);
        }
        if max_score == 0 || min_score == u64::MAX { return 0; }
        // ARITH-02 : saturating_sub + checked_div
        let diff = max_score.saturating_sub(min_score);
        diff.saturating_mul(1000).checked_div(max_score.max(1)).unwrap_or(0)
    }
    ...
    pub fn pressure_zone(&self, node: usize) -> PressureZone {
        if node >= MAX_NUMA_NODES { return PressureZone::Normal; }
        let capacity = self.node_capacity_bytes[node].load(Ordering::Relaxed);
        if capacity == 0 { return PressureZone::Normal; }
    ...
    pub fn highest_pressure_node(&self) -> usize {
        ...
        while i < MAX_NUMA_NODES {
            ...
            if capacity > 0 {
                ...
                if ppt > best_ppt { best_ppt = ppt; best = i; }
            }
            i = i.wrapping_add(1);
        }
        best
...
mod tests {
    ...
    fn fresh_policy() -> NumaPolicy { NumaPolicy::new_const() }
    ...
    fn test_set_node_capacity_ok() {
        ...
        assert_eq!(p.node_capacity_bytes[0].load(Ordering::Relaxed), 8_000_000_000);
    }
    ...
    fn test_report_is_balanced() {
        ...
        let r = p._make_report(
            TuningEvent::NoChange, 0, PlacementStrategy::RoundRobin,
