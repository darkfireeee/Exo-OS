...
impl NumaNodeStats {
    ...
    pub fn net_allocs(&self) -> u64 { self.allocs.saturating_sub(self.frees) }
    /// Octets nets alloués, ARITH-02.
    pub fn net_bytes(&self) -> u64  { self.bytes_alloc.saturating_sub(self.bytes_freed) }
    ...
    pub fn usage_ppt(&self, capacity_bytes: u64) -> u64 {
        ...
        self.net_bytes().saturating_mul(1000)
            .checked_div(capacity_bytes).unwrap_or(1000)
    }
    /// Bandwidth write en octets/tick, ARITH-02.
    pub fn write_bw(&self, ticks_elapsed: u64) -> u64 {
        if ticks_elapsed == 0 { return 0; }
    ...
    pub fn is_under_pressure(&self) -> bool { self.pressure_events > 0 }
...
impl NumaStats {
    ...
    pub fn record_migration(&self, from: usize, to: usize) {
        if self.guard(from) { self.migrations_out[from].fetch_add(1, Ordering::Relaxed); }
        if self.guard(to)   { self.migrations_in[to].fetch_add(1, Ordering::Relaxed); }
    }
    ...
    pub fn node_stats(&self, node: usize) -> NumaNodeStats {
        if !self.guard(node) { return NumaNodeStats::default(); }
        NumaNodeStats {
    ...
    pub fn least_loaded_node(&self) -> usize {
        ...
        while i < MAX_NUMA_NODES {
            ...
            if s < best_score { best_score = s; best_node = i; }
            i = i.wrapping_add(1);
        }
        best_node
    }
    ...
    pub fn imbalance_ppt(&self) -> u64 {
        ...
        if min_score == u64::MAX { min_score = 0; }
        max_score.saturating_sub(min_score)
            .saturating_mul(1000)
            .checked_div(max_score).unwrap_or(0)
    }
    ...
    pub fn reset_all(&self) {
        let mut i = 0usize;
        while i < MAX_NUMA_NODES { self.reset_node(i); i = i.wrapping_add(1); }
    }
}
