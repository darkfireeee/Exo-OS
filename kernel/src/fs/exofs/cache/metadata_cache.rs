...
impl MetadataCache {
    pub const fn new_const() -> Self {
        Self {
            inner: SpinLock::new(MetadataCacheInner::new(32768)),
        }
    }
...
impl MetadataCache {
    ...
    pub fn stats(&self) -> MetaCacheStats {
        ...
        for m in inner.map.values() {
            if m.dirty {
                n_dirty = n_dirty.wrapping_add(1);
            }
            match m.kind {
            ...
            if m.cached_tick < oldest {
                oldest = m.cached_tick;
            }
            if m.cached_tick > newest {
                newest = m.cached_tick;
            }
        }
        MetaCacheStats {
