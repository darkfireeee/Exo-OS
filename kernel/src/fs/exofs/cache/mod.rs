...
pub use cache_eviction::{EvictionAlgorithm, EvictionPolicy};
pub use cache_policy::{CacheConfig, CachePolicy};
...
pub use object_cache::{CachedObject, ObjectCache, OBJECT_CACHE};
...
mod tests {
    ...
    #[test]
    fn test_verify_health_returns_report() {
        init();
        let r = verify_health();
        assert!(r.is_healthy());
    }
    ...
    #[test]
    fn test_update_pressure_low_when_empty() {
        init();
        CACHE_PRESSURE.set_max_bytes(256 * 1024 * 1024);
    ...
    #[test]
    fn test_health_report_total_entries() {
        init();
        let r = verify_health();
    ...
    #[test]
    fn test_health_report_hit_ratio_zero_when_idle() {
        init();
        let r = verify_health();
    ...
    #[test]
    fn test_reclaim_bytes_no_panic() {
        init();
...
impl ReclaimReport {
    ...
    pub fn target_met(&self) -> bool {
        self.bytes_freed >= self.bytes_requested
    }
}

/// Orchestre un reclaim avec contrôle de l'agressivité.
pub fn reclaim_with_report(bytes: u64, aggression: ReclaimAggression) -> ReclaimReport {
...
pub fn auto_pressure_response() -> Option<ReclaimReport> {
    ...
    match level {
        cache_pressure::PressureLevel::Critical => Some(reclaim_with_report(
            64 * 1024 * 1024,
            ReclaimAggression::Aggressive,
        )),
        cache_pressure::PressureLevel::High => Some(reclaim_with_report(
            32 * 1024 * 1024,
            ReclaimAggression::Moderate,
        )),
        cache_pressure::PressureLevel::Medium => Some(reclaim_with_report(
            8 * 1024 * 1024,
            ReclaimAggression::Conservative,
        )),
        _ => None,
    }
}
...
mod extra_tests {
    ...
    #[test]
    fn test_reclaim_report_target_met() {
        let r = ReclaimReport {
            bytes_requested: 10,
            bytes_freed: 10,
            ..Default::default()
    ...
    #[test]
    fn test_reclaim_with_report_conservative() {
        init();
    ...
    #[test]
    fn test_reclaim_with_report_aggressive() {
        init();
    ...
    #[test]
    fn test_auto_pressure_response_no_panic() {
        init();
        CACHE_PRESSURE.set_max_bytes(256 * 1024 * 1024);
        CACHE_PRESSURE.update_default(0);
    ...
    #[test]
    fn test_should_reclaim_false_when_empty() {
        init();
        CACHE_PRESSURE.set_max_bytes(256 * 1024 * 1024);
        CACHE_PRESSURE.update_default(0);
    ...
    #[test]
    fn test_tick_returns_health_report() {
        init();
        let r = tick();
        assert!(r.is_healthy());
    }
    ...
    #[test]
    fn test_capacity_config_default_valid() {
    ...
    #[test]
    fn test_health_report_is_healthy_initially() {
        init();
        assert!(verify_health().is_healthy());
    }

    #[test]
    fn test_health_total_used_bytes() {
        init();
        let r = verify_health();
