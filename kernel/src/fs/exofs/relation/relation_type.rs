...
impl RelationKind {
    ...
    pub fn is_content_sharing(self) -> bool {
        matches!(self, Self::Dedup | Self::Clone | Self::Snapshot | Self::SnapshotBase)
    }
...
impl RelationWeight {
    ...
    #[inline]
    pub fn is_strong(self) -> bool { self.0 >= Self::STRONG.0 }
}
...
impl RelationFlags {
    ...
    pub fn is_deleted(self) -> bool { self.has(Self::DELETED) }
    pub fn is_verified(self) -> bool { self.has(Self::VERIFIED) }
    pub fn is_temporary(self) -> bool { self.has(Self::TEMPORARY) }
}
...
impl RelationType {
    ...
    pub fn weight_u32(self) -> u32 { self.weight.0 }

    /// Drapeaux u16 pour la sérialisation.
    pub fn flags_u16(self) -> u16 { self.flags.0 }
}

impl Default for RelationType {
    fn default() -> Self { Self::new(RelationKind::CrossRef) }
}
...
impl RelationFilter {
    ...
    pub fn matches(self, rt: RelationType) -> bool {
        if self.active_only && !rt.is_active() { return false; }
        if let Some(k) = self.kind {
            if rt.kind != k { return false; }
        }
        if let Some(min_w) = self.min_weight {
            if rt.weight.0 < min_w { return false; }
        }
        true
    }
}
...
mod tests {
    ...
    #[test] fn test_weight_saturating() {
        let w = RelationWeight(u32::MAX);
        assert_eq!(w.saturating_add(RelationWeight(1)), RelationWeight::MAX);
        assert_eq!(RelationWeight(0).saturating_sub(RelationWeight(5)), RelationWeight::ZERO);
    }

    #[test] fn test_flags_operations() {
    ...
    #[test] fn test_is_hierarchical() {
        assert!(RelationKind::Parent.is_hierarchical());
        assert!(!RelationKind::Dedup.is_hierarchical());
    }

        #[test] fn test_label_completeness() {
        let kinds = [
            RelationKind::Parent, RelationKind::Child, RelationKind::Symlink,
            RelationKind::HardLink, RelationKind::Refcount, RelationKind::Snapshot,
            RelationKind::SnapshotBase, RelationKind::Dedup, RelationKind::Clone,
            RelationKind::CrossRef, RelationKind::DataDep, RelationKind::Metadata,
        ];
        for k in kinds { assert!(!k.label().is_empty()); }
    }

    #[test] fn test_strong_dependency() {
        assert!(RelationKind::Refcount.is_strong_dependency());
        assert!(!RelationKind::Symlink.is_strong_dependency());
    }

    #[test] fn test_content_sharing() {
    ...
    #[test] fn test_directed() {
        assert!(RelationKind::Parent.is_directed());
        assert!(!RelationKind::Dedup.is_directed());
    }

    #[test] fn test_filter_active_only() {
        let f = RelationFilter { active_only: true, ..Default::default() };
        let deleted = RelationType::new(RelationKind::Clone)
            .with_flag(RelationFlags::DELETED);
        assert!(!f.matches(deleted));
    }

    #[test] fn test_weight_is_strong() {
