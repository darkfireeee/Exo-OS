...
impl CompressionHeader {
    ...
    pub fn from_bytes(buf: &[u8]) -> ExofsResult<Self> {
        ...
        let magic = u32::from_le_bytes(
            buf[0..4].try_into().map_err(|_| ExofsError::CorruptedStructure)?
        );
    ...
    pub fn payload<'a>(&self, full_buf: &'a [u8]) -> ExofsResult<&'a [u8]> {
        ...
        let end   = start.checked_add(self.compressed_size as usize)
            .ok_or(ExofsError::OffsetOverflow)?;
        full_buf.get(start..end).ok_or(ExofsError::CorruptedStructure)
    }
}
...
impl<'a> CompressedBlobView<'a> {
    ...
    /// Checksum CRC32 des données compressées.
    pub fn crc32(&self) -> u32 { self.header.crc32 }
}
...
mod tests {
    ...
    #[test] fn test_header_size_is_32() {
        assert_eq!(core::mem::size_of::<CompressionHeader>(), 32);
    }

    #[test] fn test_new_and_is_valid() {
    ...
    #[test] fn test_roundtrip_to_from_bytes() {
...
mod header_builder_tests {
    ...
    #[test] fn test_builder_roundtrip() {
        let h  = HeaderBuilder::new()
