...
use crate::fs::exofs::compress::compress_header::{CompressionHeader, COMPRESSION_MAGIC, COMPRESSION_HEADER_SIZE};
...
impl DecompressReader {
    ...
    pub const fn new() -> Self {
        Self { stats: DecompressStats::new(), validate_crc: true }
    }

    /// Crée un lecteur sans validation CRC (mode rapide).
    pub const fn fast_mode() -> Self {
        Self { stats: DecompressStats::new(), validate_crc: false }
    }
    ...
    pub fn decompress_with_header(&mut self, payload: &[u8]) -> ExofsResult<Vec<u8>> {
        ...
        let header = CompressionHeader::from_bytes(
            payload[..COMPRESSION_HEADER_SIZE].try_into()
        ...
        let result = match algo {
            CompressionAlgorithm::None => {
                let mut out = Vec::new();
                out.try_reserve(expected_cs).map_err(|_| ExofsError::NoMemory)?;
                out.extend_from_slice(&compressed[..expected_cs]);
                self.stats.raw = self.stats.raw.saturating_add(1);
                out
            }
            CompressionAlgorithm::Lz4 => {
                let out = Lz4Compressor::decompress_to_vec(
                    &compressed[..expected_cs], orig_size
                )?;
                self.stats.lz4 = self.stats.lz4.saturating_add(1);
                out
            }
            CompressionAlgorithm::Zstd => {
                let out = ZstdCompressor::decompress_to_vec(
                    &compressed[..expected_cs], orig_size
                )?;
    ...
    pub fn decompress_raw(
        ...
        let result = match algo {
            ...
            CompressionAlgorithm::Lz4 =>
                Lz4Compressor::decompress_to_vec(data, orig_size)?,
            CompressionAlgorithm::Zstd =>
                ZstdCompressor::decompress_to_vec(data, orig_size)?,
        };
        self.stats.total = self.stats.total.saturating_add(1);
        Ok(result)
    }
    ...
    pub fn stats(&self) -> &DecompressStats { &self.stats }

    /// Réinitialise les statistiques.
    pub fn reset_stats(&mut self) { self.stats = DecompressStats::new(); }

    // ── Méthode interne pour payload brut ─────────────────────────────────────

    fn decompress_raw_copy(&mut self, payload: &[u8]) -> ExofsResult<Vec<u8>> {
        let mut out = Vec::new();
        out.try_reserve(payload.len()).map_err(|_| ExofsError::NoMemory)?;
...
mod tests {
    ...
    #[test] fn test_is_compressed_false_empty() {
        assert!(!DecompressReader::is_compressed(&[]));
    }

    #[test] fn test_is_compressed_false_short() {
    ...
    #[test] fn test_crc_error_rate_zero_when_no_errors() {
        let r = DecompressReader::new();
        assert_eq!(r.stats().crc_error_rate(), 0.0);
    }

    #[test] fn test_bad_struct_size() {
    ...
    #[test] fn test_decompress_raw_none() {
        let data = b"direct data";
        let mut r = DecompressReader::new();
        let out   = r.decompress_raw(data, CompressionAlgorithm::None, data.len()).unwrap();
        assert_eq!(out.as_slice(), data);
    }

    #[test] fn test_fast_mode_no_crc_error() {
    ...
    #[test] fn test_crc_error_rate_formula() {
    ...
    #[test] fn test_decompress_raw_lz4() {
        let data = uniform(512);
        let mut comp = Vec::new();
        Lz4Compressor::compress(&data, &mut comp).unwrap();
        let mut r  = DecompressReader::new();
        let dec    = r.decompress_raw(&comp, CompressionAlgorithm::Lz4, data.len()).unwrap();
        assert_eq!(dec, data);
    }

    #[test] fn test_decompress_raw_zstd() {
        let data  = uniform(256);
        let comp  = ZstdCompressor::compress_to_vec(&data, 3).unwrap();
        let mut r = DecompressReader::new();
        let dec   = r.decompress_raw(&comp, CompressionAlgorithm::Zstd, data.len()).unwrap();
        assert_eq!(dec, data);
    }

    #[test] fn test_total_increments_on_raw_passthrough() {
        let mut r = DecompressReader::new();
    ...
    #[test] fn test_stats_raw_counter() {
        let mut r = DecompressReader::new();
        let _ = r.decompress(b"plain").unwrap();
        assert!(r.stats().raw >= 1);
    }
    #[test] fn test_stats_struct_errors_zero_initially() {
        let r = DecompressReader::new();
        assert_eq!(r.stats().struct_errors, 0);
    }
    #[test] fn test_decompress_small_raw_no_magic() {
        let data = b"small";
        let mut r = DecompressReader::new();
        let out   = r.decompress(data).unwrap();
        assert_eq!(out.len(), data.len());
    }
    #[test] fn test_fast_mode_correct_flag() {
    ...
    #[test] fn test_is_not_compressed_when_wrong_magic() { assert!(!DecompressReader::is_compressed(&[0x00u8; 16])); }
    #[test] fn test_stats_raw_no_header() { let mut r = DecompressReader::new(); let _ = r.decompress(b"xyzw no magic here at all").unwrap(); assert!(r.stats().raw >= 1); }
    #[test] fn test_total_zero_initially() { let r = DecompressReader::new(); assert_eq!(r.stats().total, 0); }
}
