...
impl SnapshotHeaderDisk {
    ...
    #[inline]
    pub fn parent_id_opt(&self) -> Option<u64> {
        if self.parent_id == 0 {
            None
        } else {
            Some(self.parent_id)
...
impl Phase3Context {
    ...
    fn register(&mut self, sid: u64, lba: u64, flags: u8, depth: u32) -> ExofsResult<()> {
    ...
    fn contains(&self, sid: u64) -> bool {
        self.seen.contains_key(&sid)
    }
    /// Retourne les métadonnées associées.
    #[inline]
    fn get(&self, sid: u64) -> Option<(u64, u8)> {
        self.seen.get(&sid).copied()
    }
    /// Retourne la profondeur calculée.
    #[inline]
    fn depth_of(&self, sid: u64) -> Option<u32> {
        self.depths.get(&sid).copied()
    }
}
...
impl FsckPhase3 {
    ...
    pub fn run_with_options(
        ...
        'scan: for i in 0..opts.scan_max {
            ...
            if errors.len() as u32 >= opts.max_errors {
                break 'scan;
            }
        }
...
mod tests {
    ...
    fn test_hdr_build_roundtrip() {
        ...
        let hdr = SnapshotHeaderDisk::build(42, 0, 7, 12345, root_blob, 1024, 4, name, 0);
