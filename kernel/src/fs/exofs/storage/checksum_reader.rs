...
use crate::fs::exofs::storage::checksum_writer::{
    split_framed, verify_checksum, ChecksumTag, CHECKSUM_MAGIC, CHECKSUM_TAG_LEN,
};
use alloc::vec::Vec;
...
impl<'a> ChecksumReader<'a> {
    ...
    pub fn verify(&self) -> ExofsResult<VerifyResult> {
        ...
        data.try_reserve(data_slice.len())
    ...
    pub fn data_len(&self) -> usize {
        self.framed.len() - CHECKSUM_TAG_LEN
    }
}
...
impl ChecksumValidationReport {
    ...
    pub fn add_ok(&mut self, offset: DiskOffset) -> ExofsResult<()> {
        self.results
            .try_reserve(1)
            .map_err(|_| ExofsError::NoMemory)?;
        self.results.push(BlockValidationResult {
            offset,
            ok: true,
            reason: None,
        });
    ...
    pub fn total(&self) -> u64 {
        self.ok_count.saturating_add(self.fail_count)
    }
...
impl BatchFrameVerifier {
    pub fn verify_all(frames: &[(DiskOffset, &[u8])]) -> ExofsResult<ChecksumValidationReport> {
        ...
        for (offset, framed) in frames {
            match ChecksumReader::new(framed) {
                ...
                Ok(reader) => match reader.data_verified() {
                    Ok(_) => {
                        report.add_ok(*offset)?;
                    }
                    Err(_) => {
                        report.add_fail(*offset, "checksum_mismatch")?;
                    }
                },
            }
        }
