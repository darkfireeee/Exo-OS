...
impl ExtentReader {
    ...
    pub fn read(
        ...
        if size == 0 { return Err(ExofsError::InvalidArgument); }
        ...
        Ok(ExtentReadResult { data: buf, extent, bytes_read: n as u64, cached: false, segments: 1 })
    }
    ...
    pub fn read_blocks(
        ...
        out.try_reserve(total_sz).map_err(|_| ExofsError::NoMemory)?;
        ...
        while remaining > 0 {
            ...
            blk_buf.try_reserve(to_read).map_err(|_| ExofsError::NoMemory)?;
        ...
        Ok(ExtentReadResult { data: out, extent, bytes_read, cached: false, segments: seg_count })
    }
    ...
    pub fn read_cached(
        ...
        let extent = Extent::new(offset, n as u64);
        Ok(ExtentReadResult { data: buf, extent, bytes_read: n as u64, cached: true, segments: 1 })
    }
    ...
    pub fn read_cached_blocks(
        ...
        for _ in 0..n_blocks {
            ...
            buf_blk.try_reserve(BLOCK_SIZE as usize).map_err(|_| ExofsError::NoMemory)?;
        ...
        let extent = Extent::new(offset, bytes_read);
        Ok(ExtentReadResult { data: out, extent, bytes_read, cached: true, segments: seg_count })
    }

    pub fn total_bytes_read(&self) -> u64 { self.bytes_read.load(Ordering::Relaxed) }
    pub fn total_read_ops(&self)   -> u64 { self.read_ops.load(Ordering::Relaxed) }
    pub fn cache_ops(&self)        -> u64 { self.cache_ops.load(Ordering::Relaxed) }
    pub fn error_count(&self)      -> u64 { self.errors.load(Ordering::Relaxed) }
}

impl Default for ExtentReader { fn default() -> Self { Self::new() } }
...
pub fn scatter_read(
    ...
    out.try_reserve(extents.len()).map_err(|_| ExofsError::NoMemory)?;

    for (i, extent) in extents.iter().enumerate() {
        match reader.read(extent.offset, extent.size, read_fn) {
            Ok(r)  => { out.try_reserve(1).map_err(|_| ExofsError::NoMemory)?;
                        out.push(ScatterReadItem { extent_index: i, data: r.data, ok: true }); }
            Err(_) => { out.try_reserve(1).map_err(|_| ExofsError::NoMemory)?;
                        out.push(ScatterReadItem { extent_index: i, data: Vec::new(), ok: false }); }
        }
    }
    Ok(out)
}
...
mod tests {
    ...
    fn mock_read_eof(_off: DiskOffset, _buf: &mut [u8]) -> ExofsResult<usize> { Ok(0) }
...
impl ExtentReader {
    ...
    pub fn gather_read(
        ...
        let mut out: Vec<u8> = Vec::new();
        out.try_reserve(total_cap as usize).map_err(|_| ExofsError::NoMemory)?;

        for ext in extents {
            let r = self.read(ext.offset, ext.size, read_fn)?;
            out.try_reserve(r.data.len()).map_err(|_| ExofsError::NoMemory)?;
    ...
    pub fn read_partial(
        ...
        let abs_off = DiskOffset(extent.offset.0.checked_add(rel_off).ok_or(ExofsError::Overflow)?);
