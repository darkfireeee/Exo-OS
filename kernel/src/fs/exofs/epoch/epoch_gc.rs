...
use crate::fs::exofs::core::{
    ExofsError, ExofsResult, EpochId, ObjectId, DiskOffset,
};
...
impl GcEpochWindow {
    ...
    pub fn contains(&self, epoch: EpochId) -> bool {
        !self.is_empty()
            && epoch.0 >= self.from_epoch.0
            && epoch.0 < self.until_epoch.0
    }
}
...
impl DeferredDeleteEntry {
    ...
    pub fn new(
        ...
        Self { object_id, del_epoch, data_offset, size_bytes, reason }
    }
}
...
impl DeferredDeleteQueue {
    ...
    pub fn drain_collectable(
        ...
        collectables.try_reserve(n).map_err(|_| ExofsError::NoMemory)?;
...
impl GcStats {
    ...
    #[inline] pub fn inc_cycles(&self)                       { self.cycles.fetch_add(1, Ordering::Relaxed); }
    #[inline] pub fn add_epochs_freed(&self, n: u64)         { self.epochs_freed.fetch_add(n, Ordering::Relaxed); }
    #[inline] pub fn add_objects_freed(&self, n: u64)        { self.objects_freed.fetch_add(n, Ordering::Relaxed); }
    #[inline] pub fn add_entries_drained(&self, n: u64)      { self.entries_drained.fetch_add(n, Ordering::Relaxed); }
    #[inline] pub fn inc_cycles_blocked(&self)               { self.cycles_blocked.fetch_add(1, Ordering::Relaxed); }
    #[inline] pub fn add_bytes_freed(&self, n: u64)          { self.bytes_freed.fetch_add(n, Ordering::Relaxed); }
