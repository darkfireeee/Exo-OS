...
impl EpochRecoveryStats {
    ...
    #[inline] pub fn inc_slot_io_errors(&self)    { self.slot_io_errors.fetch_add(1, Ordering::Relaxed); }
    #[inline] pub fn inc_checksum_errors(&self)   { self.checksum_errors.fetch_add(1, Ordering::Relaxed); }
    #[inline] pub fn inc_degraded_mounts(&self)   { self.degraded_mounts.fetch_add(1, Ordering::Relaxed); }
    #[inline] pub fn inc_slot_magic_errors(&self) { self.slot_magic_errors.fetch_add(1, Ordering::Relaxed); }
...
impl LatencyHistogram {
    ...
    pub fn record(&self, cycles: u64) {
        ...
        let _ = self.min_cycles.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |old| {
            if cycles < old { Some(cycles) } else { None }
        });
        // Mise à jour max.
        let _ = self.max_cycles.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |old| {
            if cycles > old { Some(cycles) } else { None }
        });
    }
...
impl EpochStats {
    ...
    #[inline] pub fn inc_partial_commits(&self)                    { self.partial_commits.fetch_add(1, Ordering::Relaxed); }
    ...
    #[inline] pub fn inc_barrier_failures(&self)                   { self.barrier_failures.fetch_add(1, Ordering::Relaxed); }
    ...
    #[inline] pub fn inc_roots_with_deletions(&self)               { self.roots_with_deletions.fetch_add(1, Ordering::Relaxed); }
    #[inline] pub fn inc_root_page_errors(&self)                   { self.root_page_errors.fetch_add(1, Ordering::Relaxed); }

    // ── Increments recovery ────────────────────────────────────────────────
    #[inline] pub fn inc_recovery_slot_io_errors(&self)            { self.recovery.inc_slot_io_errors(); }
    #[inline] pub fn inc_recovery_checksum_errors(&self)           { self.recovery.inc_checksum_errors(); }
    #[inline] pub fn inc_recovery_degraded_mounts(&self)           { self.recovery.inc_degraded_mounts(); }
    #[inline] pub fn inc_recovery_slot_magic_errors(&self)         { self.recovery.inc_slot_magic_errors(); }
    #[inline] pub fn inc_recovery_epochs_replayed(&self)           { self.recovery.inc_epochs_replayed(); }

    // ── Increments pins ───────────────────────────────────────────────────
    #[inline] pub fn inc_pins_acquired(&self)                      { self.pins_acquired.fetch_add(1, Ordering::Relaxed); }
    #[inline] pub fn inc_pins_failed(&self)                        { self.pins_failed.fetch_add(1, Ordering::Relaxed); }
    #[inline] pub fn update_pin_max(&self, cur: u64)               {
        let _ = self.pin_max_concurrent.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |old| {
            if cur > old { Some(cur) } else { None }
        });
    }

    // ── Increments snapshots ──────────────────────────────────────────────
    #[inline] pub fn inc_snapshots_created(&self)                  { self.snapshots_created.fetch_add(1, Ordering::Relaxed); }
    #[inline] pub fn inc_snapshots_deleted(&self)                  { self.snapshots_deleted.fetch_add(1, Ordering::Relaxed); }

    // ── Increments writeback ──────────────────────────────────────────────
    #[inline] pub fn inc_writeback_timer_flush(&self)              { self.writeback_timer_flushes.fetch_add(1, Ordering::Relaxed); }
    #[inline] pub fn inc_writeback_pressure_flush(&self)           { self.writeback_pressure_flushes.fetch_add(1, Ordering::Relaxed); }
    #[inline] pub fn inc_writeback_full_flush(&self)               { self.writeback_full_flushes.fetch_add(1, Ordering::Relaxed); }

    // ── Increments GC ─────────────────────────────────────────────────────
    #[inline] pub fn inc_gc_cycles(&self)                          { self.gc_cycles.fetch_add(1, Ordering::Relaxed); }
    #[inline] pub fn add_gc_epochs_collected(&self, n: u64)        { self.gc_epochs_collected.fetch_add(n, Ordering::Relaxed); }
    #[inline] pub fn add_gc_objects_freed(&self, n: u64)           { self.gc_objects_freed.fetch_add(n, Ordering::Relaxed); }

    // ── Latence ───────────────────────────────────────────────────────────
    #[inline] pub fn record_commit_cycles(&self, cycles: u64)      { self.commit_latency.record(cycles); }
    ...
    pub fn health_score(&self) -> u8 {
        ...
        if abort_pct > 50 { return 20; }
        if abort_pct > 20 { return 50; }
        if abort_pct > 5  { return 75; }
        if snp.barrier_failures > 0 { return 80; }
        100
    }
}
