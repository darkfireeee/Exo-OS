...
impl DedupStats {
    ...
    pub fn record_blob(
        ...
        if deduped {
            self.deduped_blobs.fetch_add(1, Ordering::Relaxed);
        }
        self.total_chunks_processed
            .fetch_add(n_chunks, Ordering::Relaxed);
        self.deduped_chunks
            .fetch_add(deduped_chunks, Ordering::Relaxed);
...
mod tests {
    ...
    #[test]
    fn test_record_blob_with_dedup() {
        let s = DedupStats::new_const();
    ...
    #[test]
    fn test_global_stats_accessible() {
...
impl DedupStatsDelta {
    ...
    pub fn compute(before: &DedupStatsSummary, after: &DedupStatsSummary) -> Self {
        let blobs_delta =
            (after.total_blobs_processed as i64).wrapping_sub(before.total_blobs_processed as i64);
        let chunks_delta = (after.total_chunks_processed as i64)
            .wrapping_sub(before.total_chunks_processed as i64);
        let saved_bytes_delta = (after.saved_bytes as i64).wrapping_sub(before.saved_bytes as i64);
        let dedup_ratio_change =
            (after.dedup_ratio_pct as i8).wrapping_sub(before.dedup_ratio_pct as i8);
        Self {
            blobs_delta,
            chunks_delta,
            saved_bytes_delta,
...
impl DedupEfficiencyMetrics {
    pub fn from_summary(s: &DedupStatsSummary) -> Self {
        let amp = if s.physical_bytes_stored == 0 {
            100
        } else {
        ...
        let bpc = if s.total_chunks_processed == 0 {
            0
        } else {
        ...
        let cpb = if s.total_blobs_processed == 0 {
            0
        } else {
...
mod tests_advanced {
    ...
    #[test]
    fn test_history_invalid_capacity() {
