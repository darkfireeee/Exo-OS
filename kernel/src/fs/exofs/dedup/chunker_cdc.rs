...
impl RollingHash {
    fn new() -> Self {
        Self { window: [0u8; WINDOW_SIZE], wpos: 0, hash: 0 }
    }
    ...
    fn roll(&mut self, byte: u8) -> u64 {
        ...
        let h        = if h >= term_old { h - term_old } else { h + RABIN_MOD - term_old };
        let h        = h + term_new;
...
impl CdcConfig {
    ...
    pub fn validate(&self) -> ExofsResult<()> {
        if self.min_size < CHUNK_MIN_SIZE      { return Err(ExofsError::InvalidArgument); }
        if self.max_size > CHUNK_MAX_SIZE      { return Err(ExofsError::InvalidArgument); }
        if self.min_size >= self.max_size      { return Err(ExofsError::InvalidArgument); }
        if self.avg_size < self.min_size
        || self.avg_size > self.max_size       { return Err(ExofsError::InvalidArgument); }
        Ok(())
    }

    /// Masque CDC calculé depuis avg_size.
    fn mask(&self) -> u64 { (self.avg_size as u64).saturating_sub(1) }
}
...
impl CdcChunker {
    ...
    pub fn default_chunker() -> Self {
        Self { config: CdcConfig::default() }
    }

    pub fn with_sizes(min: usize, avg: usize, max: usize) -> ExofsResult<Self> {
        Self::new(CdcConfig { min_size: min, avg_size: avg, max_size: max, include_data: true })
    }
...
impl Chunker for CdcChunker {
    ...
    fn boundaries(&self, data: &[u8]) -> ExofsResult<Vec<ChunkBoundary>> {
        ...
        let mut pos: usize = 0;
        while pos < total {
            if bounds.len() >= CHUNK_MAX_PER_BLOB { return Err(ExofsError::OffsetOverflow); }
            let len = self.find_boundary(data, pos);
    ...
    fn min_chunk_size(&self) -> usize { self.config.min_size }
    fn max_chunk_size(&self) -> usize { self.config.max_size }
}
...
impl CdcStats {
    ...
    pub fn compute(chunks: &[DedupChunk], config: &CdcConfig) -> ExofsResult<Self> {
        ...
        Ok(Self { base, forced_max_cuts: forced, natural_cuts: natural })
    }
}
...
mod tests {
    ...
    fn cdc() -> CdcChunker { CdcChunker::default_chunker() }

    #[test] fn test_cdc_empty_data() {
        assert!(cdc().chunk(&[]).unwrap().is_empty());
    }

    #[test] fn test_cdc_small_data_single_chunk() {
    ...
    #[test] fn test_cdc_boundaries_cover_entire_data() {
    ...
    #[test] fn test_cdc_deterministic() {
    ...
    #[test] fn test_cdc_fingerprints_differ() {
    ...
    #[test] fn test_cdc_invalid_config() {
    ...
    #[test] fn test_mulmod61_no_overflow() {
...
mod tests_adaptive {
    ...
    #[test] fn test_sharpness_configs_valid() {
