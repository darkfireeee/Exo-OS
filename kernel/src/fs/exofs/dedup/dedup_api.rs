...
impl DedupResult {
    fn new(
        ...
        let ratio = if logical_bytes == 0 {
            ...
            ((saved * 100) / logical_bytes).min(100) as u8
        };
        Self {
            blob_id,
            is_new,
...
impl DedupBatchReport {
    ...
    pub fn from_results(results: &[DedupResult]) -> Self {
        ...
        for r in results {
            ...
            if !r.is_new || r.deduped_chunks > 0 {
                dedup_b = dedup_b.saturating_add(1);
            }
...
mod tests {
    ...
    #[test]
    fn test_dedup_ratio_returns_u8() {
        let api = api();
    ...
    #[test]
    fn test_dedup_result_with_savings() {
