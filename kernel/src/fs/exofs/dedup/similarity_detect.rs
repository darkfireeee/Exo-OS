...
impl BlobSignature {
    ...
    pub fn from_chunk_hashes(blob_id: BlobId, chunk_hashes: &[[u8; 32]]) -> ExofsResult<Self> {
        if chunk_hashes.is_empty() {
            let mut sig: Vec<u64> = Vec::new();
            sig.try_reserve(MIN_HASH_COUNT)
                .map_err(|_| ExofsError::NoMemory)?;
            for i in 0..MIN_HASH_COUNT {
                sig.push(i as u64);
            }
            return Ok(Self {
                blob_id,
        ...
        buf.try_reserve(chunk_hashes.len() * 8)
            .map_err(|_| ExofsError::NoMemory)?;
...
impl SimilarityDetector {
    ...
    pub fn default() -> Self {
        Self {
            threshold_pct: SIMILARITY_THRESHOLD_PCT,
        }
    }

    /// Compare deux blobs et retourne un `SimilarityMatch` si similaires.
    pub fn compare(&self, sig_a: &BlobSignature, sig_b: &BlobSignature) -> Option<SimilarityMatch> {
    ...
    pub fn find_similar_to(
        ...
        let mut results: Vec<SimilarityMatch> = Vec::new();
        for c in candidates {
            if c.blob_id.as_bytes() == reference.blob_id.as_bytes() {
                continue;
            }
    ...
    pub fn find_and_rank(&self, signatures: &[BlobSignature]) -> ExofsResult<Vec<SimilarityMatch>> {
...
mod tests {
    ...
    #[test]
    fn test_similar_data_above_threshold() {
