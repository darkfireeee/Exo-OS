...
impl FixedChunker {
    ...
    pub fn new(chunk_size: usize) -> ExofsResult<Self> {
        let config = FixedChunkerConfig { chunk_size, ..FixedChunkerConfig::default() };
        config.validate()?;
        Ok(Self { config })
    }
    ...
    pub fn default_chunker() -> Self {
        Self { config: FixedChunkerConfig::default() }
    }
    ...
    pub fn chunk_count_for(&self, data_len: usize) -> ExofsResult<usize> {
        ...
        // ceil(data_len / cs)
        let count = data_len.checked_add(cs.wrapping_sub(1))
            .map(|v| v / cs)
            .ok_or(ExofsError::OffsetOverflow)?;
        if count > CHUNK_MAX_PER_BLOB { return Err(ExofsError::OffsetOverflow); }
        Ok(count)
    }
    ...
    pub fn compute_boundaries_only(&self, data: &[u8]) -> ExofsResult<Vec<ChunkBoundary>> {
        ...
        bounds.try_reserve(count).map_err(|_| ExofsError::NoMemory)?;
...
impl Chunker for FixedChunker {
    ...
    fn chunk(&self, data: &[u8]) -> ExofsResult<Vec<DedupChunk>> {
        ...
        chunks.try_reserve(count).map_err(|_| ExofsError::NoMemory)?;
        let mut offset: usize = 0;
        while offset < total {
            ...
            let fast_hash = if self.config.compute_fast_hash { fnv1a64(raw) } else { 0 };
    ...
    fn min_chunk_size(&self) -> usize { self.config.chunk_size }
    fn max_chunk_size(&self) -> usize { self.config.chunk_size }
}
...
impl BatchFixedChunker {
    pub fn new(chunk_size: usize) -> ExofsResult<Self> {
        Ok(Self { inner: FixedChunker::new(chunk_size)? })
    }
    ...
    pub fn chunk_batch(&self, blobs: &[&[u8]]) -> ExofsResult<Vec<FixedChunkResult>> {
        ...
        for (idx, &blob) in blobs.iter().enumerate() {
            ...
            results.push(FixedChunkResult { blob_idx: idx, chunks, stats });
        }
        Ok(results)
    }
    ...
    pub fn all_fingerprints(&self, blobs: &[&[u8]]) -> ExofsResult<Vec<ChunkFingerprint>> {
        ...
        for &blob in blobs {
            let chunks = self.inner.chunk(blob)?;
            fps.try_reserve(chunks.len()).map_err(|_| ExofsError::NoMemory)?;
            for c in chunks {
...
mod tests {
    ...
    fn chunker() -> FixedChunker { FixedChunker::new(4).unwrap() }

    #[test] fn test_chunk_count() {
    ...
    #[test] fn test_chunk_produces_correct_count() {
        let c      = chunker();
        let data   = &[0u8; 10];
    ...
    #[test] fn test_last_chunk_undersized() {
    ...
    #[test] fn test_chunk_empty_data() {
        let c = chunker();
        assert!(c.chunk(&[]).unwrap().is_empty());
    }

    #[test] fn test_chunk_data_preserved() {
    ...
    #[test] fn test_boundaries_match_chunk() {
        let c      = chunker();
        let data   = &[0u8; 12];
    ...
    #[test] fn test_chunk_blake3_deterministic() {
        let c      = chunker();
        let data   = &[0xABu8; 8];
    ...
    #[test] fn test_invalid_chunk_size_zero() {
        assert!(FixedChunker::new(0).is_err());
    }

    #[test] fn test_invalid_chunk_size_too_large() {
        assert!(FixedChunker::new(FIXED_MAX_CHUNK_SIZE + 1).is_err());
    }

    #[test] fn test_batch_chunker() {
    ...
    #[test] fn test_batch_all_fingerprints() {
        let bc  = BatchFixedChunker::new(4).unwrap();
        let fps = bc.all_fingerprints(&[&[0u8; 8]]).unwrap();
        assert_eq!(fps.len(), 2);
    }

    #[test] fn test_no_data_mode() {
        let cfg = FixedChunkerConfig {
            chunk_size: 4, include_data: false,
            pad_last_chunk: false, compute_fast_hash: false,
        };
...
impl SlidingWindowChunker {
    ...
    pub fn slide_fingerprints(&self, data: &[u8]) -> ExofsResult<Vec<ChunkFingerprint>> {
        ...
        let mut fps: Vec<ChunkFingerprint> = Vec::new();
        fps.try_reserve(n_windows).map_err(|_| ExofsError::NoMemory)?;
        let mut pos: usize = 0;
    ...
    pub fn window_count(&self, data_len: usize) -> usize {
        if data_len < self.chunk_size { return 1; }
...
impl AlignedFixedChunker {
    ...
    pub fn chunk_aligned(&self, data: &[u8]) -> ExofsResult<Vec<DedupChunk>> {
        ...
        let n       = (total + cs - 1) / cs;
        if n > CHUNK_MAX_PER_BLOB { return Err(ExofsError::OffsetOverflow); }
