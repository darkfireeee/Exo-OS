...
impl ThroughputWindow {
    ...
    pub fn push(&self, sample: ThroughputSample) {
        ...
        self.total_read
            .fetch_add(sample.bytes_read, Ordering::Relaxed);
        self.total_written
            .fetch_add(sample.bytes_written, Ordering::Relaxed);
    }
    ...
    pub fn avg_read_bpt(&self) -> u64 {
        ...
        while i < n as usize {
            let idx = (head
                .wrapping_add(THROUGHPUT_WINDOW_SIZE)
                .wrapping_sub(i + 1))
    ...
    pub fn latest(&self) -> Option<ThroughputSample> {
        ...
        let head = self.head.load(Ordering::Relaxed) as usize;
        let idx =
            (head.wrapping_add(THROUGHPUT_WINDOW_SIZE).wrapping_sub(1)) % THROUGHPUT_WINDOW_SIZE;
        // SAFETY: accès exclusif garanti par lock atomique acquis avant.
    ...
    pub fn reset(&self) {
        ...
        while i < THROUGHPUT_WINDOW_SIZE {
            // SAFETY: accès exclusif garanti par lock atomique acquis avant.
            unsafe {
                (*self.samples.get())[i] = ThroughputSample::zero();
            }
            i = i.wrapping_add(1);
        }
...
impl ThroughputTracker {
    ...
    pub fn flush_period(&self, dt: u64) {
        ...
        if rbpt > old_r {
            self.peak_read_bpt.store(rbpt, Ordering::Relaxed);
        }
        let old_w = self.peak_write_bpt.load(Ordering::Relaxed);
        if wbpt > old_w {
            self.peak_write_bpt.store(wbpt, Ordering::Relaxed);
        }
        self.last_flush_tick.store(tick, Ordering::Relaxed);
    }

    pub fn avg_read_bpt(&self) -> u64 {
        self.window.avg_read_bpt()
    }
    pub fn avg_write_bpt(&self) -> u64 {
        self.window.avg_write_bpt()
    }
    pub fn avg_total_bpt(&self) -> u64 {
        self.window.avg_total_bpt()
    }
    pub fn peak_read_bpt(&self) -> u64 {
        self.peak_read_bpt.load(Ordering::Relaxed)
    }
    pub fn peak_write_bpt(&self) -> u64 {
        self.peak_write_bpt.load(Ordering::Relaxed)
    }
    ...
    pub fn pending_read_bytes(&self) -> u64 {
        self.bytes_read.load(Ordering::Relaxed)
    }
    pub fn pending_write_bytes(&self) -> u64 {
        self.bytes_written.load(Ordering::Relaxed)
    }
    pub fn last_flush_tick(&self) -> u64 {
        self.last_flush_tick.load(Ordering::Relaxed)
    }
...
impl ThroughputRate {
    pub fn between(a: &ThroughputSample, b: &ThroughputSample) -> Self {
        ...
        Self {
            read_bps: b
                .bytes_read
                .saturating_sub(a.bytes_read)
                .checked_div(dt)
                .unwrap_or(0),
            write_bps: b
                .bytes_written
                .saturating_sub(a.bytes_written)
                .checked_div(dt)
                .unwrap_or(0),
            dt_ticks: dt,
        }
...
impl ThroughputThresholds {
    ...
    pub fn check_read(&self, bpt: u64) -> bool {
        bpt <= self.max_read_bpt
    }
    pub fn check_write(&self, bpt: u64) -> bool {
        bpt <= self.max_write_bpt
    }
    pub fn is_active(&self, bpt: u64) -> bool {
        bpt >= self.min_active_bpt
    }
}
...
mod tests {
    ...
    fn test_rate_between_read_heavy() {
        let a = ThroughputSample {
            tick: 0,
            bytes_read: 0,
            bytes_written: 0,
        };
        let b = ThroughputSample {
            tick: 10,
            bytes_read: 1000,
            bytes_written: 200,
        };
