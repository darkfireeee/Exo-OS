...
impl DebugCommand {
    ...
    pub fn command_id(&self) -> DebugCommandId {
...
impl DebugResponse {
    ...
    pub fn status(&self) -> DebugResponseStatus {
        DebugResponseStatus::from_u8(self.status)
    }
    pub fn is_ok(&self) -> bool {
        self.status().is_ok()
    }
...
impl DebugQueue {
    ...
    pub fn cmd_pending(&self) -> u64 {
        self.cmd_count.load(Ordering::Relaxed)
    }
    pub fn resp_pending(&self) -> u64 {
        self.resp_count.load(Ordering::Relaxed)
    }
    pub fn total_cmds(&self) -> u64 {
        self.total_cmds.load(Ordering::Relaxed)
    }
    pub fn is_empty(&self) -> bool {
        self.cmd_pending() == 0
    }
...
impl<'a> DebugSession<'a> {
    ...
    pub fn execute(&self, cmd: DebugCommand) -> DebugResponse {
        ...
        let resp = match id {
            ...
            DebugCommandId::GetStatus => {
                DebugResponse::ok(id, self.exec_cnt.load(Ordering::Relaxed), "status ok")
            }
    ...
    pub fn exec_count(&self) -> u64 {
        self.exec_cnt.load(Ordering::Relaxed)
    }
...
impl DebugStats {
    ...
    pub fn is_full(&self) -> bool {
        self.cmd_pending >= self.queue_size as u64 || self.resp_pending >= self.queue_size as u64
    }
}
