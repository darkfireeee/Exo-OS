...
impl LatencyHistogram {
    ...
    pub fn record(&self, us: u64) {
        ...
        while us < cur_min {
            match self.min_us.compare_exchange_weak(cur_min, us, Ordering::Relaxed, Ordering::Relaxed) {
                Ok(_) => break,
                Err(v) => cur_min = v,
            }
        }
        ...
        while us > cur_max {
            match self.max_us.compare_exchange_weak(cur_max, us, Ordering::Relaxed, Ordering::Relaxed) {
                Ok(_) => break,
                Err(v) => cur_max = v,
            }
        }
    ...
    pub fn sum_us(&self) -> u64 { self.sum_us.load(Ordering::Relaxed) }
    pub fn min_us(&self) -> u64 {
        let v = self.min_us.load(Ordering::Relaxed);
        if v == u64::MAX { 0 } else { v }
    }
    pub fn max_us(&self) -> u64 { self.max_us.load(Ordering::Relaxed) }

    /// Moyenne en µs (ARITH-02).
    pub fn mean_us(&self) -> u64 {
        let c = self.count.load(Ordering::Relaxed);
        self.sum_us.load(Ordering::Relaxed).checked_div(c).unwrap_or(0)
    }
    ...
    pub fn percentile_us(&self, pct: u8) -> u64 {
        ...
        if total == 0 { return 0; }
        let target = total.saturating_mul(pct as u64).checked_div(100).unwrap_or(0);
        let snap = self.bucket_snapshot();
        let mut cumul = 0u64;
        let mut i = 0usize;
        while i < NUM_BUCKETS {
            cumul = cumul.saturating_add(snap[i]);
            if cumul >= target { return BUCKET_UPPER_US[i]; }
            i = i.wrapping_add(1);
        }
...
impl LatencySummary {
    ...
    pub fn p99_ok(&self, threshold_us: u64) -> bool { self.p99_us <= threshold_us }
}
...
impl LatencyWindow {
    pub const fn new_const() -> Self {
        #[allow(clippy::declare_interior_mutable_const)]
        const Z: AtomicU64 = AtomicU64::new(0);
        Self { samples: [Z; LATENCY_WINDOW_SIZE], head: AtomicU64::new(0), count: AtomicU64::new(0) }
    }
    ...
    pub fn push(&self, us: u64) {
        ...
        let c = self.count.load(Ordering::Relaxed);
        if c < LATENCY_WINDOW_SIZE as u64 { self.count.fetch_add(1, Ordering::Relaxed); }
    }
    ...
    pub fn to_vec(&self) -> ExofsResult<Vec<u64>> {
        ...
        let mut i = 0usize;
        while i < n {
            let idx = (head.wrapping_add(LATENCY_WINDOW_SIZE).wrapping_sub(i).wrapping_sub(1)) % LATENCY_WINDOW_SIZE;
...
mod tests {
    ...
    fn test_percentile_p50() {
        ...
        let mut i = 0usize;
        while i < 4 { h.record(1); i = i.wrapping_add(1); }
        let mut j = 0usize;
        while j < 4 { h.record(16); j = j.wrapping_add(1); }
    ...
    fn test_summary_peak_bucket() {
        let h = LatencyHistogram::new_const();
        let mut i = 0usize;
        while i < 5 { h.record(500); i = i.wrapping_add(1); }
        h.record(1);
        let s = h.summary();
    ...
    #[test]
    fn test_p99_ok_fn() {
        let s = LatencySummary { p99_us: 50, ..Default::default() };
        assert!(s.p99_ok(100));
        assert!(!s.p99_ok(49));
    }
}
