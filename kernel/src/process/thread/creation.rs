...
pub fn create_thread(params: &ThreadCreateParams) -> Result<ThreadHandle, ThreadCreateError> {
    ...
    // 1. Allouer TID.
    let tid_raw = TID_ALLOCATOR.alloc()
    ...
    let thread = ProcessThread::new(
        tid, pcb.pid, cr3, params.attr.policy, params.attr.priority,
    ).ok_or_else(|| {
    ...
    unsafe {
        ...
        *frame.add(3) = params.attr.stack_addr
    ...
    Ok(ThreadHandle { tid, thread: thread_ptr })
}
