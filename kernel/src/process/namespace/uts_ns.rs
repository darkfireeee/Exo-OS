...
impl UtsNamespace {
    const fn new_root() -> Self {
        ...
        // "exo-os" en ASCII
        hostname[0] = b'e'; hostname[1] = b'x'; hostname[2] = b'o';
        hostname[3] = b'-'; hostname[4] = b'o'; hostname[5] = b's';
        Self {
            id:             0,
    ...
    pub fn set_hostname(&self, src: &[u8]) -> bool {
        if src.len() >= HOSTNAME_MAX { return false; }
        let _g = self.lock.lock();
