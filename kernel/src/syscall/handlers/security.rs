...
pub fn sys_check_capability(cap_type: CapabilityType, target: u64) -> MemoryResult<bool> {
    log::debug!(
        "sys_check_capability: type={:?}, target={}",
        cap_type,
        target
    );
    ...
    if let Some(cap_list) = caps.get(&current_pid) {
        ...
        for cap in cap_list {
            if cap.valid && cap.cap_type == cap_type && (cap.target == target || cap.target == 0) {
                return Ok(true);
            }
        }
    }
    ...
    log::debug!(
        "check_capability: DENIED for PID {} {:?}",
        current_pid,
        cap_type
    );
    Ok(false)
...
pub fn sys_grant_capability(
    ...
    log::debug!(
        "sys_grant_capability: pid={}, type={:?}, target={}",
        pid,
        cap_type,
        target
    );
    ...
    caps.entry(pid).or_insert_with(Vec::new).push(capability);

    log::info!(
        "grant_capability: PID {} granted {:?} for target {}",
        pid,
        cap_type,
        target
    );
...
pub fn sys_revoke_capability(pid: u64, cap_id: CapId) -> MemoryResult<()> {
    ...
    if let Some(cap_list) = caps.get_mut(&pid) {
        for cap in cap_list.iter_mut() {
            if cap.id == cap_id {
                cap.valid = false;
                log::info!("revoke_capability: revoked cap {} from PID {}", cap_id, pid);
                return Ok(());
            }
        }
    }
...
pub fn sys_transfer_capability(target_pid: u64, cap_id: CapId) -> MemoryResult<()> {
    log::debug!(
        "sys_transfer_capability: target_pid={}, cap_id={}",
        target_pid,
        cap_id
    );
...
pub fn sys_setuid(uid: u64) -> MemoryResult<()> {
    ...
    let proc_creds = all_creds.entry(current_pid).or_insert(creds);
...
pub fn sys_seccomp(mode: u32, flags: u32, filter: usize) -> MemoryResult<()> {
    log::debug!(
        "sys_seccomp: mode={}, flags={}, filter={:#x}",
        mode,
        flags,
        filter
    );
...
pub fn sys_unveil(path: &str, permissions: &str) -> MemoryResult<()> {
    ...
    let (list, locked) = unveils.entry(current_pid).or_insert((Vec::new(), false));
...
static PROCESS_NAMES: Mutex<BTreeMap<u64, alloc::string::String>> = Mutex::new(BTreeMap::new());
static PROCESS_DUMPABLE: Mutex<BTreeMap<u64, i32>> = Mutex::new(BTreeMap::new());

/// Process control (prctl)
pub unsafe fn sys_prctl(option: i32, arg2: usize, arg3: usize, arg4: usize, arg5: usize) -> i64 {
    log::info!(
        "sys_prctl: option={}, arg2={:#x}, arg3={:#x}, arg4={:#x}, arg5={:#x}",
        option,
        arg2,
        arg3,
        arg4,
    ...
    match option {
        PR_SET_NAME => {
            ...
            PROCESS_NAMES
                .lock()
                .insert(current_pid, alloc::string::String::from(name));
            log::info!("prctl: PID {} set name to '{}'", current_pid, name);
            0
        }
        ...
        PR_GET_NAME => {
            ...
            for i in 0..copy_len {
                *name_ptr.add(i) = name_bytes[i];
            }
            *name_ptr.add(copy_len) = 0; // Null terminator
        ...
        PR_SET_DUMPABLE => {
            ...
            PROCESS_DUMPABLE.lock().insert(current_pid, dumpable);
            log::info!("prctl: PID {} set dumpable to {}", current_pid, dumpable);
            0
        }
        ...
        PR_GET_DUMPABLE => {
            ...
            let dumpable = PROCESS_DUMPABLE
                ...
                .get(&current_pid)
                .copied()
                .unwrap_or(1); // Default to dumpable
            dumpable as i64
        }

        PR_SET_PDEATHSIG => {
            // Set parent death signal
            log::info!("prctl: PID {} set pdeathsig to {}", current_pid, arg2);
            // TODO: Store parent death signal
            0
        }
        ...
        PR_SET_SECCOMP => {
            // Set seccomp mode
            log::info!("prctl: PID {} set seccomp mode to {}", current_pid, arg2);
            sys_seccomp(arg2 as u32, 0, arg3).map(|_| 0).unwrap_or(-1)
        }

        PR_GET_SECCOMP => {
            // Get seccomp mode
            let mode = SECCOMP_MODES.lock().get(&current_pid).copied().unwrap_or(0);
            mode as i64
        }

        _ => {
            log::warn!("prctl: unsupported option {}", option);
            -22 // EINVAL
        }
    }
}
