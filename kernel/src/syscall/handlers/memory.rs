...
pub fn sys_mmap(
    ...
    let protection = if prot.read && prot.write && prot.exec {
        crate::memory::PageProtection::ReadWriteExecute
    } else if prot.read && prot.write {
        crate::memory::PageProtection::ReadWrite
    } else if prot.read && prot.exec {
        crate::memory::PageProtection::ReadExecute
    } else if prot.read {
        crate::memory::PageProtection::ReadOnly
    } else {
        crate::memory::PageProtection::NoAccess
    };
    
    let mmap_flags = crate::memory::mmap::MmapFlags::new(
        (if flags.shared { crate::memory::mmap::MmapFlags::SHARED } else { 0 }) |
        (if flags.private { crate::memory::mmap::MmapFlags::PRIVATE } else { 0 }) |
        (if flags.fixed { crate::memory::mmap::MmapFlags::FIXED } else { 0 }) |
        (if flags.anonymous { crate::memory::mmap::MmapFlags::ANONYMOUS } else { 0 })
    );
    
    // 3. Call mmap manager
    let fd_opt = if flags.anonymous { None } else { Some(fd as i32) };
    let addr_opt = if addr.value() != 0 { Some(addr) } else { None };
    
    let result = crate::memory::mmap::mmap(
        addr_opt,
        length,
        protection,
        mmap_flags,
    ...
    log::info!("mmap: allocated {:?}, size {}, prot={:?}", result, length, prot);
    Ok(result)
}
...
pub fn sys_mprotect(addr: VirtualAddress, length: usize, prot: ProtFlags) -> MemoryResult<()> {
    log::debug!("sys_mprotect: addr={:?}, len={}, prot={:?}", addr, length, prot);
    ...
    log::info!("mprotect: changed {:?}, size {}, prot={:?}", addr, length, prot);
    Ok(())
}

/// Sync memory to disk (msync)
pub fn sys_msync(addr: VirtualAddress, length: usize, flags: u32) -> MemoryResult<()> {
    log::debug!("sys_msync: addr={:?}, len={}, flags={}", addr, length, flags);
...
pub fn sys_madvise(addr: VirtualAddress, length: usize, advice: i32) -> MemoryResult<()> {
    log::debug!("sys_madvise: addr={:?}, len={}, advice={}", addr, length, advice);
...
pub fn sys_mlock(addr: VirtualAddress, length: usize) -> MemoryResult<()> {
    ...
    if locked.len() + page_count > MAX_LOCKED_PAGES {
        log::warn!("mlock: would exceed limit ({} + {} > {})",
            locked.len(), page_count, MAX_LOCKED_PAGES);
...
pub fn sys_mremap(
    ...
    if new_size < old_size {
        ...
        log::info!("mremap: shrunk mapping from {} to {} bytes", old_size, new_size);
        return Ok(old_addr);
    }
    ...
    if new_size > old_size {
        ...
        if flags & MREMAP_MAYMOVE != 0 {
            ...
            let result = sys_mmap(
                ...
                MapFlags { shared: false, private: true, fixed: flags & MREMAP_FIXED != 0, anonymous: true },
                0,
                0,
            )?;
            
            // Copy old data (stub - would use memcpy)
            log::info!("mremap: moved from {:?} to {:?}, size {}", old_addr, result, new_size);
