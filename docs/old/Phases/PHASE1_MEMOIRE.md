...
Étape 4  '4' _  TSS per-CPU BSP + IST stacks (fait dans init_gdt_for_cpu)
...
| ✅ | HPET MMIO remappé avec UC | **Implémenté** | `fixmap_slot_addr(FIXMAP_HPET)` + `PAGE_FLAGS_MMIO` — fallback identity map sur QEMU |
| ✅ | TSC calibré via HPET | **Implémenté** | Chaîne HPET→PM→CPUID→PIT→3GHz dans `calibration/mod.rs` (FIX TIME-02) |
...
| `MEM-03` | TSC fallback 3 GHz → délais faux sur hardware réel | ✅ Corrigé — calibration par fenêtre réelle HPET/PM/PIT (`time/calibration/`) |
...
|---|---|---|---|
| ✅ Résolu | Calibration TSC via fenêtre HPET/PM/PIT | `arch/x86_64/time/calibration/` | `ERR-01` corrigé |
| ✅ Résolu | HPET MMIO remappé avec `PAGE_FLAGS_MMIO` + fixmap | `arch/x86_64/acpi/hpet.rs` | `MEM-01` complet |
