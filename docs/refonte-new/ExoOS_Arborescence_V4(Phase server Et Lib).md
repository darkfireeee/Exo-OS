...
Version 4  ·  Mars 2026  ·  ExoOS Project
Corrections V1→V2→V3→V4 intégrées — validé par analyse multi-modèles (6 IA × 3 tours)


Changelog V4 — Corrections appliquées
V4 est une version de finition. Les 3 corrections sont ciblées et non bloquantes en V3 — mais nécessaires pour la cohérence documentaire et l'intégrité du déploiement ExoPhoenix.

Réf.	Correction V4	Origine
V4-1	scheduler_server ajouté à la table section 7 (étape 12, exo_shield décalé à 13)	Détecté par Z-AI + Gemini — omission dans table de création
V4-2	Pipeline enregistrement binaires dans ExoFS (build/register_binaries.sh) documenté	Détecté par Gemini + Copilote — gap architectural forge.rs
V4-3	Format sealed blob crypto_server/isolation.rs spécifié : ChaCha20Poly1305 + clé dérivée	Détecté par Z-AI, Gemini, Kimi, MiniMax — ambiguïté d'implémentation
NOTE	scheduler_server et network_server intentionnellement SANS isolation.rs — état transient	Clarification préventive — Kimi avait raison
...
11	scheduler_server	init_server disponible	Politique scheduling Ring 1/3
...
📄 ✦ isolation.rs	Handler PrepareIsolation — scelle les clés actives : ChaCha20Poly1305(clé dérivée de master_key) → blob → EXOFS_WRITE → ObjectId → PrepareIsolationAck ✦ NOUVEAU
...
ℹ scheduler_server et network_server n'ont PAS de isolation.rs — intentionnel. Leur état est transient et recalculable : la politique de scheduling est statique, les sockets réseau peuvent être refermées et réouvertes. Lors d'un cycle ExoPhoenix, ces servers sont simplement relancés depuis zéro sans perte de données critiques.
...
Pipeline d'enregistrement des binaires dans ExoFS ✦ V4 (requis pour forge.rs)
ExoPhoenix (forge.rs) recharge les servers et drivers depuis ExoFS après chaque cycle de reconstruction. Pour que ce mécanisme fonctionne, chaque binaire ELF doit être enregistré dans ExoFS avec son hash Blake3 comme ObjectId. Ce pipeline s'exécute au moment du déploiement initial (premier boot ou CI).
ℹ PHX-03 : tout binaire server/driver déployé DOIT avoir son hash Blake3(ELF) enregistré dans ExoFS. Sans cette étape, forge.rs ne peut pas reconstruire le système après un cycle ExoPhoenix.

# build/register_binaries.sh — à exécuter après cargo build --release
# Calcule le hash Blake3 de chaque ELF et l'enregistre dans ExoFS

for binary in target/release/ipc_broker init_server vfs_server \
              memory_server crypto_server device_server \
              scheduler_server network_server \
              virtio_block virtio_net virtio_console; do
  HASH=$(b3sum --no-names $binary)
  # Appel syscall EXOFS_WRITE_BINARY via outil de déploiement
  exofs-deploy register --binary $binary --hash $HASH
done

# Résultat : chaque ELF est indexé dans ExoFS par ObjectId = Blake3(ELF)
# forge.rs utilise ces ObjectId pour recharger les binaires après reconstruction
✦ V3 — exofs-deploy est un outil de déploiement (hors ExoOS, côté host) qui écrit les binaires et leurs métadonnées dans l'image ExoFS initiale. À créer dans build/ en parallèle de l'implémentation Phase 1.


7. Ordre de création — à donner à GPT-5.3-Codex
Cet ordre est la seule table de création valide. 13 étapes. device_server PRÉCÈDE tous les drivers. scheduler_server est à l'étape 12 (correction V4). exo_shield est à l'étape 13.
⚠ device_server (étape 8) DOIT précéder virtio-block (étape 9). Un driver ne peut pas exécuter Claim si device_server n'existe pas.
...
12	servers/scheduler_server ✦	init_server disponible	SetPriority/Yield opérationnels — politique scheduling Ring 1/3
13	servers/exo_shield	Phase 3 ExoPhoenix stable	PrepareIsolation flow complet : broadcast → ack → phoenix_notify(521)
...
PHX-03 ✦	Binaires dans ExoFS	Chaque binaire ELF doit avoir son Blake3(ELF) enregistré dans ExoFS via build/register_binaries.sh avant premier démarrage. Requis pour forge.rs.

ExoOS — Arborescence Servers + Drivers  ·  Version 4  ·  Mars 2026
