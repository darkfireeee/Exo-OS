...
- [ ] **2.1** `integrity_check` — 🔍 **AUDITÉ : crypto RÉELLE (ed25519+blake3) mais 3 chemins non-câblés.**
  - **runtime_check** ([runtime_check.rs](../../kernel/src/security/integrity_check/runtime_check.rs)) : hash blake3 `.text/.rodata` réel, compare en temps constant. `init_runtime_integrity()` calcule les refs au boot ✓. ✅ **2.1-a FAIT** : kthread moniteur `integrity-mon` ([mod.rs](../../kernel/src/security/integrity_check/mod.rs) `start_integrity_monitor`, lancé après la phase SECURITY de [lib.rs](../../kernel/src/lib.rs)) — dort 15 s, puis (une fois `is_security_ready` → `.text` stable) appelle `security_periodic_check_observe()` = **mode observe** : sur divergence → **ExoLedger P0** (tag `"INTEGRTY"`) + compteur, **JAMAIS de panic** (un faux-positif ne fait qu'un log → boot-safe). Mode fatal (`assert_kernel_integrity` → panic+handoff) réservé prod derrière flag. **PAS** dans le tick IRQ (hash de tout `.text` en interruption = latence prohibitive). 75 tests sécurité verts, compile + lib OK.
  ...
  - **Conclusion** : **2.1-a FAIT** (moniteur observe-mode). Restent **2.1-b** (exo-boot doit produire une BootAttestation signée + kernel appelle `verify_boot_attestation` early-boot — **cross-composant** avec le bootloader) et **2.1-c** (autorité de signature des binaires `/system` = **PKI build-time réelle** : clé privée provisionnée au déploiement, non falsifiable en code ; le crypto_server fournit le primitive de vérif mais la signature kernel reste crypto kernel). Ces 2 gaps sont **déploiement-dépendants** (bootloader + PKI), pas du code à « débloquer » unilatéralement.
...
- [x] **2.10** ✅ **isolation — syscall `pledge()` CÂBLÉ.** Infra réelle (SandboxPolicy 256-bit + PledgeSet OpenBSD, [sandbox.rs](../../kernel/src/security/isolation/sandbox.rs)/[pledge.rs](../../kernel/src/security/isolation/pledge.rs)) **+ point d'entrée userspace ajouté** : `SYS_EXO_PLEDGE=324` ([numbers.rs](../../kernel/src/syscall/numbers.rs)) → `sys_exo_pledge` ([table.rs](../../kernel/src/syscall/table.rs)) → `pledge_promises_to_restrictions` ([process_state.rs](../../kernel/src/security/zero_trust/process_state.rs)) → `restrict_process(caller_pid, …)`. Mapping : promesse `PROC` absente → `NO_FORK|NO_PROCESS_CREATE`, `EXEC` absent → `NO_EXEC`, `INET` absent → `NO_NETWORK`. **Monotone** (durcit seulement), refuse init. **Enforcement déjà réel** (TIER 1.1 `syscall_restriction_mask`) → un process qui `pledge()` voit ses syscalls interdits refusés (`DenyAndAudit`). Test `pledge_maps_absent_promises_to_restrictions` + e2e `syscall_restriction_is_enforced`. *Reste (extension)* : pledges FS (rpath/wpath) quand l'enforcement FS par-syscall existera.
...
- **Validation** : `cargo check -p exo-crypto-server` OK + **build ISO bare-metal OK** (ISO_EXIT=0).

## TIER 2 — état final

| # | Module | État |
|---|--------|------|
| 2.1 | integrity_check | 🟡 **2.1-a FAIT** (moniteur intégrité observe-mode, kthread) ; restent 2.1-b/c = **PKI/bootloader déploiement-dépendants** |
| 2.2–2.6 | exoseal/exoveil/exocage/exoargos/exonmi | ✅ réels (IOMMU/PKS/CET/PMC/watchdog HW), câblés boot+ledger+handoff |
| 2.7 | exoledger | ✅ réel + câblé (chaîné Blake3, P0) |
| 2.8 | exokairos | ✅ réel + câblé (HMAC-Blake3 deadline) |
| 2.9 | exploit_mitigations | ✅ réel (CET/MSR/KASLR) |
| 2.10 | isolation | ✅ **syscall `pledge()` câblé** (SYS_EXO_PLEDGE=324 → restrict_process → enforcement TIER 1.1) |
| 2.11 | audit | ✅ réel + câblé (capte refus TIER 0/1) |
| 2C | crypto_server + crypto kernel | ✅ **durci** (RNG multi-sources, ChaCha20 KAT, tls/pki CSPRNG, rotate_key HKDF) |

**TIER 2 = réel & validé**, hors 2 gaps déploiement-dépendants (2.1-b/c PKI+bootloader) et 1 feature userspace (2.10 `pledge()`). Aucun scaffold/placeholder restant dans le code kernel/serveur sous notre contrôle.
- 2026-06-15 : **2.1-a FAIT** — kthread `integrity-mon` (observe-mode, 15 s, gaté `is_security_ready`, log ExoLedger sans panic). 75 tests sécurité verts + ISO bare-metal en validation. TIER 2 entièrement audité & réel ; reste PKI build-time (2.1-b/c) + `pledge()` (2.10).
